...
export default function SquadClient() {
  ...
  /* Transfer planner state. Declared with the other hooks: React counts them, so a hook placed after the
     loading return changes the count between renders and tears the page down. */
  const [plan, setPlan] = React.useState(null);
  const [planning, setPlanning] = React.useState(false);
  /* The planner's own horizon, kept apart from the one the pitch is showing. Judging a transfer over the
     next five weeks while looking at GW1 is a normal thing to want. */
  const [planFrom, setPlanFrom] = React.useState(null);
  const [planTo, setPlanTo] = React.useState(null);
  /* Players the planner must sell, and players it must not touch. */
  const [mustSell, setMustSell] = React.useState([]);
  const [mustKeep, setMustKeep] = React.useState([]);
  /* "rebuild" lets the solver pick a fresh eleven every week. "shape" drops the incoming player into the
     outgoing player's place and leaves the rest of the plan exactly as written. */
  const [planMode, setPlanMode] = React.useState("rebuild");
   ...
   *
   * The question every week is not "what is the best fifteen" but "what is the best fifteen I can reach
   * from the one I own". Those are different problems and only the second one is useful once the season
   * is running, because you cannot rebuild, you can only change one or two.
   *
   * The solver has always been able to answer it: give it the squad you hold and a cap on how many
   * players may differ, and it returns the optimal result within that cap, across the whole range at
   * once rather than the next fixture. So this asks it twice, for one change and for two, and reports
   * both against doing nothing.
   *
   * Two transfers costs four points unless a second is free, so the second option is only worth taking
   * when it beats the first by more than the hit. That subtraction is done here rather than left to the
   * reader, because comparing a raw total against a hit is exactly where these decisions go wrong. */

  const planRange = { from: planFrom ?? gwFrom, to: planTo ?? gwTo };

  const askSolver = (maximumChanges) => fetch("/api/exact-squad", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      gw_from: planRange.from,
      gw_to: planRange.to,
      budget: PLAN_RULES.budget,
      chip_schedule: Object.fromEntries((rangeProjection?.weekly || [])
        .filter((week) => week.chip && week.gw >= planRange.from && week.gw <= planRange.to)
        .map((week) => [String(week.gw), week.chip])),
      current_squad: (state?.players || []).map((player) => Number(player.fpl_id)),
      maximum_changes: maximumChanges,
      /* Must-sell is expressed as an exclusion, so the solver may not put him back. Must-keep is a
         keep instruction, so he survives whatever else changes. */
      ignores: mustSell,
      keep: mustKeep,
    }),
   ...
   * every week. That is the right answer if you intend to manage the side weekly. If you do not, it
   * overstates what you will actually score, because it assumes a level of rotation you will not do.
   *
   * So this scores the same transfer a second way: the incoming player takes the outgoing player's exact
   * place in every week already planned, starting when he started and benched when he was benched. The
   * rest of the plan is untouched. It uses the same xPTS as the rest of the screen, nothing new. */
  const scoreInPlace = (out, incoming) => {
    if (!shaped || !model) return null;
    let total = 0;
    for (let gameweek = planRange.from; gameweek <= planRange.to; gameweek += 1) {
      const stored = shaped.weeks?.[String(gameweek)] || shaped.weeks?.[gameweek];
      const starters = stored?.startingIds?.length
        ? stored.startingIds.map(Number)
        : (state?.players || []).filter((player) => player.starting).map((player) => Number(player.fpl_id));
      const captain = Number(stored?.captain ?? state?.captain);
      const chip = stored?.chip || null;
      for (const player of state?.players || []) {
        const id = Number(player.fpl_id);
        const swapped = out.some((leaving) => Number(leaving.fpl_id) === id);
        const who = swapped ? incoming.find((arriving) => arriving.position === player.position) : player;
        if (!who) continue;
        const plays = starters.includes(id) || chip === "benchboost";
        if (!plays) continue;
        const score = Number(model.scoreForGw(swapped ? { ...player, ...who } : player, gameweek) ?? 0);
        total += score * (captain === id ? 2 : 1);
      }
    }
    return total;
  };

  const planTransfers = async () => {
    if (!state || state.players.length !== PLAN_RULES.squadSize) {
      setPlanError("A complete fifteen is needed before transfers can be planned.");
      return;
    }
    setPlanning(true);
    setPlanError(null);
    /* One, two and three changes. Three matters because a straight swap is often impossible: the player
       you want costs more than the one you are selling, so a second or third move exists only to free the
       money. The solver handles that on its own, because it is choosing a whole fifteen under a budget
       rather than pairing players off, and it will happily sell a midfielder you can afford to downgrade
       in order to fund the forward you actually want. */
    const [hold, one, two, three] = await Promise.all([askSolver(0), askSolver(1), askSolver(2), askSolver(3)]);
    setPlanning(false);
    if (!hold?.ok) { setPlanError(hold?.error || "The planner did not finish."); return; }

    const free = week ? Number(week.free ?? 1) : 1;
    const baseline = Number(hold.xp ?? 0);
    const option = (result, changes) => {
      if (!result?.ok || !result.transfers?.count) return null;
      const hit = Math.max(0, changes - free) * 4;
      const rebuilt = Number(result.xp ?? 0) - baseline;
      const inPlace = planMode === "shape" ? scoreInPlace(result.transfers.out, result.transfers.in) : null;
      const gross = planMode === "shape" && inPlace !== null
        ? inPlace - (scoreInPlace([], []) ?? 0)
        : rebuilt;
      return {
        changes, hit, gross,
        net: gross - hit,
        out: result.transfers.out,
        in: result.transfers.in,
        funded: result.transfers.count > result.transfers.in.filter((player) => player.price).length ? false : true,
      };
    };
    const options = [option(one, 1), option(two, 2), option(three, 3)]
      .filter(Boolean)
      .sort((a, b) => b.net - a.net)
      .slice(0, 3);
    setPlan({ baseline, free, mode: planMode, range: { ...planRange }, options });
  };
  ...
  return (
    <div data-zeus-ui-version="core-restoration-v3" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* THE TRANSFER PLANNER, on the page where the squad already is rather than in a section of its own,
          because the answer only means anything next to the squad it applies to. */}
      {!readOnly && !empty && (
        <section className="zeus-planner" aria-label="Transfer planner">
          <div className="zeus-planner-head">
            <span style={code(12, T.xp)}>TRANSFER PLANNER</span>
            <button type="button" onClick={planTransfers} disabled={planning} className="fb-press"
              aria-label="Work out the best transfer"
              style={{ background: T.green, border: "none", ...lang(12.5, 800, "#04130A") }}>
              {planning ? "WORKING" : "FIND BEST TRANSFER"}
            </button>
          </div>

          <div className="zeus-planner-controls">
            <label className="zeus-strip-field" title="The gameweeks the transfer is judged over. Independent of the week the pitch is showing.">
              <span style={code(12)}>OVER</span>
              <select value={planRange.from} onChange={(event) => setPlanFrom(Number(event.target.value))}
                aria-label="Planner from gameweek" className="zeus-strip-select"
                style={{ background: T.card, border: `1px solid ${T.line}`, color: "#FFFFFF", ...lang(13, 700) }}>
                {Array.from({ length: lastGw - firstGw + 1 }, (_, index) => firstGw + index).map((gameweek) => (
                  <option key={gameweek} value={gameweek} style={{ background: T.card }}>GW{gameweek}</option>
                ))}
              </select>
            </label>
            <label className="zeus-strip-field">
              ...
              <select value={planRange.to} onChange={(event) => setPlanTo(Number(event.target.value))}
                aria-label="Planner to gameweek" className="zeus-strip-select"
                style={{ background: T.card, border: `1px solid ${T.line}`, color: "#FFFFFF", ...lang(13, 700) }}>
                {Array.from({ length: lastGw - firstGw + 1 }, (_, index) => firstGw + index)
                  .filter((gameweek) => gameweek >= planRange.from).map((gameweek) => (
                    <option key={gameweek} value={gameweek} style={{ background: T.card }}>GW{gameweek}</option>
                  ))}
              </select>
            </label>
            ...
            <label className="zeus-strip-field"
              title="Rebuild lets the solver pick a fresh eleven every week, which is the higher number but assumes you will manage the side weekly. Keep my shape drops the new player into the old player's exact place, starting when he started and benched when he was benched.">
              <span style={code(12)}>EACH WEEK</span>
              <select value={planMode} onChange={(event) => setPlanMode(event.target.value)}
                aria-label="Planner rotation mode" className="zeus-strip-select"
                style={{ background: T.card, border: `1px solid ${T.line}`, color: "#FFFFFF", ...lang(13, 700) }}>
          ...
          <div className="zeus-planner-picks">
            <span style={code(12)}>TAP A PLAYER TO FORCE HIM OUT, TAP AGAIN TO PROTECT HIM</span>
            <div className="zeus-planner-players">
              {(state?.players || []).map((player) => {
                const id = Number(player.fpl_id);
                const selling = mustSell.includes(id);
                const keeping = mustKeep.includes(id);
                return (
                  <button key={id} type="button" className="fb-press zeus-planner-pick"
                    aria-label={`${player.web_name}: ${selling ? "forced out" : keeping ? "protected" : "free to change"}`}
                    onClick={() => {
                      if (selling) { setMustSell((ids) => ids.filter((x) => x !== id)); setMustKeep((ids) => [...ids, id]); return; }
                      if (keeping) { setMustKeep((ids) => ids.filter((x) => x !== id)); return; }
                      setMustSell((ids) => [...ids, id]);
                    }}
                    style={{
                      background: selling ? "#3A0217" : keeping ? T.card : T.plate,
                      border: `1px solid ${selling ? T.pink : keeping ? T.green : T.line}`,
                      ...lang(12, 700, selling ? T.pink : keeping ? T.green : "#FFFFFF"),
                    }}>
                    {player.web_name}{selling ? " ·  OUT" : keeping ? " ·  KEEP" : ""}
                  </button>
                );
              })}
            </div>
          ...
          {plan && plan.options.length === 0 && (
            <span style={lang(13, 600)}>No transfer improves this squad over GW{plan.range.from}-GW{plan.range.to}. Hold.</span>
          )}

          {plan && plan.options.map((option) => (
            <div key={option.changes} className="zeus-planner-option">
              <span style={code(12, option.net > 0 ? T.green : T.pink)}>
                {option.changes} TRANSFER{option.changes === 1 ? "" : "S"}
                {option.hit ? ` · HIT -${option.hit}` : " · FREE"}
              </span>
              ...
              <span style={val(14, option.net > 0 ? T.green : T.pink)}>
                {option.net > 0 ? "+" : ""}{option.net.toFixed(2)} xPTS
              </span>
              <span style={lang(12, 600)}>
                {option.gross.toFixed(2)} gross{option.hit ? `, ${option.hit} paid for the hit` : ""}
              </span>
            </div>
          ))}

          {plan && plan.options.length > 0 && plan.options[0].net <= 0 && (
