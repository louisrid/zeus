...
/* `fromGw` is the first gameweek the caller cares about, not a filter on the calendar.
 *
 * Without it this always returned the next fixtures from today, so a table showing projections for
 * GW5 onward sat next to fixture tags for GW1: the numbers moved when the range changed and the
 * opponents did not, which quietly invites reading a GW5 projection against a GW1 opponent. Passing
 * the range start keeps the two in step. Omitting it keeps the old behaviour, which is what the
 * surfaces that genuinely mean "next up from now" want. */
export function nextFixturesForTeam(fixtures, teamById, teamId, limit = 8, fromGw = null) {
  ...
  const first = Number.isFinite(Number(fromGw)) ? Math.trunc(Number(fromGw)) : null;
  const out = [];

  for (const fixture of [...(fixtures || [])].sort(fixtureOrder)) {
    const gameweek = numberOrNull(fixture.gw);
    if (first !== null && gameweek !== null && gameweek < first) continue;
