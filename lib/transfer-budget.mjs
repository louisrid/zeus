...
/* `knownBank` is the bank the official API reports, when there is one. Same reasoning as squadMoney: the
 * starting budget less what the squad cost is only the bank of a team that has never transferred, and
 * the solver spending a budget that is out by a tenth will refuse a move that actually fits, or accept
 * one that does not. */
export function transferBudget(players, startingBudget = STARTING_BUDGET, knownBank = null) {
  ...
  const bank = Number.isFinite(suppliedBank)
    ? round1(suppliedBank)
    : round1(Math.max(0, money(startingBudget) - paid));
