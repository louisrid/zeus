...
export function validateAt(plan, gw, priceOf) {
  ...
  // Money: current squad valued at sale value, plus bank, must cover what was bought.
  const spend = s.players.reduce((a, p) => {
    const now = priceOf ? priceOf(p.fpl_id) : p.price;
    return a + Number(now ?? p.price ?? 0);
  }, 0);
  if (spend > PLAN_RULES.budget + 1e-9) {
    errors.push(`${spend.toFixed(1)} spent, budget is ${PLAN_RULES.budget.toFixed(1)}`);
  }
