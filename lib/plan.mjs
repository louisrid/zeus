...
export function transferLedger(plan, upToGw) {
   ...
   *
   * This always simulated from GW1 with one free transfer and no transfers made, so by GW3 it reported
   * three when the manager really had two. A saved plan is a draft, not a history: the transfers already
   * used in earlier weeks are not in it, so they cannot be counted. When the plan records the number it
   * genuinely starts with, that is the truth and it is used; otherwise the old simulation stands. */
  const declared = Number(plan?.free_transfers ?? plan?.freeTransfers);
  const hasDeclared = Number.isFinite(declared) && declared >= 0;
  let free = hasDeclared ? Math.min(PLAN_RULES.maxBanked, declared) : PLAN_RULES.freePerGw;
  const startGw = hasDeclared ? Number(plan?.free_transfers_gw ?? plan?.freeTransfersGw ?? 1) : 1;
  for (let gw = Number.isFinite(startGw) && startGw >= 1 ? startGw : 1; gw <= upToGw; gw++) {
    const week = (plan.weeks || {})[gw] || {};
    ...
    /* Nothing banks out of gameweek one. The squad is built for free before the first deadline, so the
     * transfer that week was never spent and was never a saved one either. Rolling it forward made every
     * later gameweek read one too high: GW3 showed three when the manager had two, and the hit on the
     * extra move was missed. Gameweek two starts from one, as the game does. */
    if (gw === 1) {
      free = PLAN_RULES.freePerGw;
    } else {
 ...
 */
/* `knownBank` is the bank the official API reports, when there is one.
 *
 * Without it the bank is derived as the starting budget less what the squad cost, which is only true for
 * a squad that has never been transferred. Every transfer breaks it: a player sold at his selling price
 * and replaced at a different cost moves money that the sum of purchase prices cannot see. On a real team
 * four transfers in, the derivation was out by a tenth, and it would drift further with every move.
 *
 * The official figure is a fact and is preferred whenever it is there. The derivation stays for a draft
 * squad that was never really bought, where there is nothing else to go on. */
export function squadMoney(players, priceOf, startingBudget = PLAN_RULES.budget, knownBank = null) {
  ...
  paid = round1(paid);
  value = round1(value);
  /* null is not zero. Number(null) is 0 and 0 is finite, so testing the converted value treats "no bank
     was supplied" as "the bank is empty" and throws the derivation away. The absence has to be checked
     before the conversion. */
  const suppliedBank = knownBank === null || knownBank === undefined || knownBank === ""
    ? null
    : Number(knownBank);
  const bank = Number.isFinite(suppliedBank)
    ? round1(suppliedBank)
    : round1(Math.max(0, startingBudget - paid));
...
/* WHICH CHIPS ARE SPENT, AND WHERE.
 *
 * The validator above has always known that a chip can be played once per half of the season, but it
 * only ever spoke up to reject a plan. The buttons themselves said nothing, so a chip already used in
 * GW2 looked exactly as available in GW14 as one never touched, and the only way to find out was to
 * play it and be told off. This reports the same fact the validator enforces, so the controls can show
 * it before the mistake rather than after.
 *
 * Each chip maps to what is known about it in the half a given gameweek falls in: the gameweek it was
 * played, or null if it is still available. */
export function chipUsage(plan, gw) {
  const half = Number(gw) <= PLAN_RULES.firstHalfEndsAfterGw ? "first" : "second";
  const usage = {};
  for (const chip of PLAN_RULES.chips) usage[chip] = null;
  for (const key of Object.keys(plan?.weeks || {})) {
    const week = Number(key);
    const chip = (plan.weeks[key] || {}).chip;
    if (!chip || !PLAN_RULES.chips.includes(chip)) continue;
    const playedIn = week <= PLAN_RULES.firstHalfEndsAfterGw ? "first" : "second";
    /* Only the half being viewed matters. A wildcard played in GW3 does not stop one in GW25, so
       showing it as spent in the second half would be a lie in the other direction. */
    if (playedIn !== half) continue;
    if (usage[chip] === null || week < usage[chip]) usage[chip] = week;
  }
  return { half, usage };
}
