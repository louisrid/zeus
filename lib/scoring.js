 ...
 */
/* TOP-RANK ALIGNMENT — DECISIONS 7.5.
 *
 *   alignment = 100 x (sum of effective ownership of my fifteen) / (sum of the fifteen highest EO)
 *
 * Effective ownership, not ownership: a player captained by half his owners counts for more, because
 * the armband doubles him. Measured against the best possible fifteen by EO, so 100 means owning
 * exactly what the top rank effectively owns.
 *
 * Interpretation is the same as template alignment and equally not higher-is-better. Returns null
 * when no snapshot exists, which is the case before a gameweek has been played.
 */
export function topRankAlignment(squad, eoByPlayerId) {
  if (!eoByPlayerId || eoByPlayerId.size === 0) return null;
  const mine = squad.players.reduce((a, p) => a + (Number(eoByPlayerId.get(p.fpl_id)) || 0), 0);
  const best = [...eoByPlayerId.values()].map(Number).sort((a, b) => b - a).slice(0, 15)
    .reduce((a, b) => a + b, 0);
  if (!best) return null;
  const missing = [...eoByPlayerId.entries()]
    .sort((a, b) => Number(b[1]) - Number(a[1]))
    .filter(([id]) => !squad.players.some((p) => p.fpl_id === id))
    .slice(0, 5)
    .map(([id, eo]) => ({ fpl_id: id, eo: Number(eo) }));
  return {
    pct: Math.round((mine / best) * 100),
...
export function templateAlignment(squad, templateFifteen) {
  ...
  return {
    ...
    unique,
    zoneFitted: false,
    zoneSource: `Target band arrives with the strategy study by ${SCHEDULE.complete.label}. Top-10k template needs the rival scraper.`,
  };
}
...
export function scoreSquad({ squad, pool, scoreOf, bestCaptainEv, templateFifteen, eoByPlayerId }) {
  ...
  return {
    ...
    topRank: topRankAlignment(squad, eoByPlayerId),
