...
export function mispriced(pool, xprice, { limit = 5 } = {}) {
  ...
  return [
    ...under.map(({ p, x }) => ({
      ...
      detail: `Costs ${round1(Number(p.price))}, worth ${x.xprice} at what a point costs across the league${x.withinPosition !== null ? `, and ${x.withinPosition} against ${p.position}s alone` : ""}.`,
    })),
    ...over.map(({ p, x }) => ({
      kind: "overpriced", player: p,
      headline: `${p.web_name} looks ${round1(Math.abs(x.gap))} expensive`,
      detail: `Costs ${round1(Number(p.price))}, worth ${x.xprice} at the league rate${x.withinPosition !== null ? `, and ${x.withinPosition} against ${p.position}s alone` : ""}.`,
    })),
  ];
}
