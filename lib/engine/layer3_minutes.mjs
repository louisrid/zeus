...
export function normaliseTeamStarts(players, cfg) {
  ...
  const free = eligible.filter((p) => !locked.includes(p) && !lockedBench.includes(p));
  ...
  const freeGoalkeepers = free.filter((p) => String(p.position || "").toUpperCase() === "GKP");
  const freeOutfield = free.filter((p) => String(p.position || "").toUpperCase() !== "GKP");
