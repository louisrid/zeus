...
/* Conditional minutes are a continuous mean, not a probability, but they need the same small-sample
   protection. The previous code trusted one or two unusual starts exactly, so a newly transferred player
   named in the XI could be locked at 100% to start and still inherit 48 minutes from a tiny old-role sample.
   Shrink the total observed starter minutes toward the league conditional-start mean. */
export function shrinkConditionalMinutes(totalMinutes, starts, leagueMean, k = 4) {
  const n = Math.max(0, Number(starts) || 0);
  const total = Math.max(0, Number(totalMinutes) || 0);
  const mean = Number(leagueMean);
  const strength = Math.max(0, Number(k) || 0);
  if (n <= 0) return Number.isFinite(mean) ? mean : null;
  const observed = total / n;
  if (!Number.isFinite(mean) || strength <= 0) return observed;
  return (total + strength * mean) / (n + strength);
}
...
export function forecastMinutes({ player, league, signal, gw, cfg }) {
  ...
  const shrunkStartMinutes = shrinkConditionalMinutes(
    player.startMinutes, player.starts, league.expMinStart, cfg.kStartMinutes ?? 4,
  );
  const expMinStart = Math.min(90, Math.max(0, shrunkStartMinutes ?? league.expMinStart ?? 0));
...
export function normaliseTeamStarts(players, cfg) {
  ...
  const locked = eligible.filter((p) => {
    const source = String(p.minutes_source || "");
    return source === "lineup-starter" || source === "lineup-carryover";
  });
