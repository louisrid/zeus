...
export function normaliseTeamStarts(players, cfg) {
  ...
  /* A PUBLISHED ELEVEN IS NOT A THING TO BE RESCALED.
   *
   * Normalisation exists because independent per-player estimates do not add up to eleven starters. When a
   * predicted eleven has been resolved for this club, they already do: eleven named players at 1.0 and the
   * rest as substitutes. Scaling that would drag a named starter below one, which is exactly the kind of
   * quiet disagreement between the engine and the screen this whole change exists to remove. */
  if (eligible.some((p) => String(p.minutes_source || "").startsWith("lineup"))) return players;
