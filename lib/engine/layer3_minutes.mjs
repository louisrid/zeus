 ...
 *
 * Starter substitution estimates create the bench-minute budget. Bench appearance chances are scaled
 * proportionally to their player-specific cameo forecasts, with each player capped by the probability he
 * is not starting. This removes the impossible 1,030-1,055 team-minute totals without giving every reserve
 * the same generic cameo chance. */
export function reconcileTeamExpectedMinutes(players, cfg, targetMinutes = 990) {
  ...
  const bench = available.filter((p) =>
    ...
    && Math.max(0, Number(p.exp_min_cameo || 0)) > 0
    && Math.max(0, 1 - Number(p.p_start || 0)) > 0
  );

  const expected = (p) => Math.max(0, Number(p.p_cameo || 0)) * Math.max(0, Number(p.exp_min_cameo || 0));
  const capacity = (p) => Math.max(0, 1 - Number(p.p_start || 0)) * Math.max(0, Number(p.exp_min_cameo || 0));
  let free = [...bench];
  ...
  for (const p of bench) {
    ...
    const cameoMinutes = Math.max(0, Number(p.exp_min_cameo || 0));
    p.p_cameo = cameoMinutes > 0 ? Math.max(0, Math.min(1 - Number(p.p_start || 0), mins / cameoMinutes)) : 0;
  }
...
export function normaliseTeamStarts(players, cfg) {
  ...
  /* A validated XI can become incomplete after hard availability is applied. Example: a named starter is
     now injured. Every unnamed squad player was correctly marked lineup-notNamed, but one of them must now
     compete for the vacated place. Restore only their pre-lineup forecast as the replacement weight. With
     a complete available XI both targets are zero, so every named bench player remains at exactly zero. */
  const replacementPool = [...forecastFree, ...lockedBench];
  for (const p of lockedBench) {
    p.p_start = Math.max(0, Number(p.pre_lineup_p_start || 0));
  }
  ...
  if (gkTarget > 0 && !freeGoalkeepers.length) throw new Error("Cannot normalise team starts: no free goalkeeper for the remaining slot");
