...
export const MINUTES_MODEL = { version: "minutes-interim-2", upgrade_date: "2026-08-01" };
...
export function normaliseTeamStarts(players, cfg) {
  ...
  if (eligible.length < 11) {
    throw new Error(`Cannot normalise team starts: only ${eligible.length} available players`);
  }

  const goalkeepers = eligible.filter((p) => String(p.position || "").toUpperCase() === "GKP");
  const outfield = eligible.filter((p) => String(p.position || "").toUpperCase() !== "GKP");
  if (goalkeepers.length >= 1 && outfield.length >= 10) {
    normaliseStartGroup(goalkeepers, 1, ceiling);
    normaliseStartGroup(outfield, 10, ceiling);
  } else {
    normaliseStartGroup(eligible, 11, ceiling);
  }

  for (const p of eligible) {
    const isGoalkeeper = String(p.position || "").toUpperCase() === "GKP";
    p.p_cameo = isGoalkeeper ? 0 : Math.max(0, Math.min(1 - p.p_start, p.p_cameo ?? 0));
