...
/** Use measured defensive volume when it exists; otherwise use the league/role prior. */
export function defensiveRateOrPrior({ rate, nineties, prior }) {
  const sample = Math.max(0, Number(nineties) || 0);
  const measured = Number(rate);
  if (sample > 0 && Number.isFinite(measured) && measured >= 0) return measured;
  const fallback = Number(prior);
