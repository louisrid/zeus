    ...
    for (const s of sides) {
      ...
      if (s.team.penAwardRate !== null && s.team.penAwardRate !== undefined) {
        ...
        for (let index = 0; index < awarded; index++) {
          ...
          const shareWeights = eligible.map((interval) => Math.max(0, Number(interval.p.penaltyShare) || 0));
          const shareIndex = categorical(rng, shareWeights);
          const rankedFallback = eligible
            .filter((interval) => (interval.p.penRank || 0) > 0)
            .sort((a, b) => a.p.penRank - b.p.penRank)[0];
          const weightedFallbackIndex = categorical(rng, eligible.map(scorerWeight));
          const equalFallbackIndex = categorical(rng, eligible.map((interval) =>
            String(interval.p.position || "").toUpperCase() === "GKP" ? 0.05 : 1
          ));
          const taker = (shareIndex >= 0 ? eligible[shareIndex] : null)
            || rankedFallback
            || (weightedFallbackIndex >= 0 ? eligible[weightedFallbackIndex] : null)
            || (equalFallbackIndex >= 0 ? eligible[equalFallbackIndex] : null)
            || null;
          if (!taker) continue;
      ...
      for (const taker of creditedPenalties) {
        const event = perPlayer.get(taker.p.player_id);
        event.goals += 1;
        event.penGoals += 1;
      }
      ...
      for (const minute of openGoalMinutes) {
        ...
        if (!eligible.length) continue;
        let scorerIndex = categorical(rng, eligible.map(scorerWeight));
        /* A data-free promoted squad can temporarily have zero scorer weights. Team goals still happened;
           never drop them from the player totals. Fall back to an equal outfield allocation until real
           player rates arrive. */
        if (scorerIndex < 0) {
          scorerIndex = categorical(rng, eligible.map((interval) =>
            String(interval.p.position || "").toUpperCase() === "GKP" ? 0.05 : 1
          ));
        }
...
export function summarise(rec, N) {
  ...
  return {
    ...
    e_pen_goals: +(rec.penGoals / N).toFixed(3),
