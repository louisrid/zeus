...
/* A no-history player should not begin at the minutes-weighted league mean, which is pulled upward by
   elite regulars. Derive a conservative replacement-level prior from the median established player at
   each position, capped at the ordinary league prior and never allowed to become zero for an outfielder. */
export function deriveConservativeNoHistoryRates(profiles = [], leagueRates = {}, minimumNineties = 10) {
  const out = { npxg90: {}, xa90: {} };
  for (const field of ["npxg90", "xa90"]) {
    for (const position of ["GKP", "DEF", "MID", "FWD"]) {
      const ordinary = Number(leagueRates?.[field]?.[position]);
      if (!Number.isFinite(ordinary) || ordinary < 0) continue;
      const values = profiles
        .filter((profile) => String(profile?.position || "").toUpperCase() === position)
        .filter((profile) => Number(profile?.nineties ?? profile?.rateNineties) >= minimumNineties)
        .map((profile) => Number(profile?.[field]))
        .filter((value) => Number.isFinite(value) && value >= 0 && (position === "GKP" || value > 0));
      const measured = median(values);
      const conservative = measured === null ? ordinary : Math.min(ordinary, measured);
      out[field][position] = position === "GKP"
        ? Math.max(0, conservative)
        : (conservative > 0 ? conservative : ordinary);
    }
  }
  return out;
}

/* The conservative promoted prior is only the prior part of the empirical-Bayes estimate. A player with
   no sample receives it in full at GW1; real Premier League minutes reduce its influence automatically,
   and the remaining time weight reaches zero at the configured endpoint. */
export function promotedRateBlendWeight({ gw, decayToGw, nineties = 0, shrinkNineties = 0 } = {}) {
  const timeWeight = promotedBlendWeight(gw, decayToGw);
  const n = Math.max(0, Number(nineties) || 0);
  const k = Math.max(0, Number(shrinkNineties) || 0);
  if (timeWeight <= 0 || k <= 0) return 0;
  return timeWeight * (k / (n + k));
}
...
export function allocateTeam({ team, lambda, priors, cfg, gw, promotedPrior }) {
  ...
  const ordinaryPriorFor = (p, field) => {
    const roleValue = p.role && cfg.roleRates?.[field]?.[p.role];
    if (Number.isFinite(Number(roleValue)) && Number(roleValue) >= 0) {
      if (p.position === "GKP" || Number(roleValue) > 0) return Math.max(0, Number(roleValue));
    }
    const positionValue = cfg.leagueRates?.[field]?.[p.position];
    if (Number.isFinite(Number(positionValue)) && Number(positionValue) >= 0) {
      if (p.position === "GKP" || Number(positionValue) > 0) return Math.max(0, Number(positionValue));
    }
    throw new Error(`Missing positive ${field} prior for ${p.position || "unknown position"}`);
  };
  const conservativePriorFor = (p, field) => {
    const value = Number(cfg.noHistoryRates?.[field]?.[p.position]);
    if (Number.isFinite(value) && value >= 0 && (p.position === "GKP" || value > 0)) return value;

    const positionValue = Number(cfg.leagueRates?.[field]?.[p.position]);
    if (Number.isFinite(positionValue) && positionValue >= 0
      && (p.position === "GKP" || positionValue > 0)) {
      return positionValue;
    }

    return ordinaryPriorFor(p, field);
  };
  ...
  const lowSampleBoundFor = (p, field) => {
    const roleValue = Number(cfg.attackingRateBounds?.[field]?.role?.[p.role]);
    if (Number.isFinite(roleValue) && roleValue >= 0) return roleValue;
    const positionValue = Number(cfg.attackingRateBounds?.[field]?.position?.[p.position]);
    return Number.isFinite(positionValue) && positionValue >= 0 ? positionValue : null;
  };
  const rateDetails = (p, field) => {
    const nineties = Math.max(0, Number(sampleFor(p, field)) || 0);
    const k = Math.max(0, Number(cfg.rateShrinkNineties ?? cfg.kPos ?? 12) || 0);
    const ordinaryPrior = ordinaryPriorFor(p, field);
    const timeWeight = team.promoted ? promotedBlendWeight(gw, cfg.promotedDecayToGw) : 0;
    const targetPrior = timeWeight > 0
      ? (1 - timeWeight) * ordinaryPrior + timeWeight * conservativePriorFor(p, field)
      : ordinaryPrior;
    const rawRate = Math.max(0, Number(p[field]) || 0);
    const sampleFloor = Math.max(0, Number(cfg.minimumRoleNineties ?? 10) || 0);
    const upperBound = nineties < sampleFloor ? lowSampleBoundFor(p, field) : null;
    const boundedRate = upperBound !== null ? Math.min(rawRate, upperBound) : rawRate;
    return {
      rate: reliableRate({ rate: boundedRate, nineties, prior: targetPrior, k }),
      priorBlend: team.promoted
        ? promotedRateBlendWeight({ gw, decayToGw: cfg.promotedDecayToGw, nineties, shrinkNineties: k })
        : 0,
      rateCapped: boundedRate + 1e-12 < rawRate,
      upperBound,
    };
  };

  const goalDetails = new Map(players.map((p) => [p.player_id, rateDetails(p, "npxg90")]));
  const assistDetails = new Map(players.map((p) => [p.player_id, rateDetails(p, "xa90")]));
  const reliableGoalRates = new Map(players.map((p) => [p.player_id, goalDetails.get(p.player_id).rate]));
  const reliableAssistRates = new Map(players.map((p) => [p.player_id, assistDetails.get(p.player_id).rate]));
  ...
  const useLegacySharePrior = blend > 0 && promotedPrior && !cfg.noHistoryRates;
  ...
  const out = players.map((p) => {
    ...
    const playerPriorBlend = Math.max(
      goalDetails.get(p.player_id)?.priorBlend || 0,
      assistDetails.get(p.player_id)?.priorBlend || 0,
    );
    const lowSampleRateBound = Boolean(
      goalDetails.get(p.player_id)?.rateCapped || assistDetails.get(p.player_id)?.rateCapped
    );
    const sourceParts = [p.rate_source || "prior-positional"];
    if (playerPriorBlend > 0) sourceParts.push("promoted-conservative");
    if (p.role_source === "predicted-formation-fallback") sourceParts.push(`lineup-role:${p.role}`);
    if (lowSampleRateBound) sourceParts.push("low-sample-upper-bound");
    return {
      ...p,
      goalShare,
      assistShare,
      finishing: finishingMultiplier(p.goals, p.xg, p.shots, cfg.finishingK, cfg.finishingClamp),
      prior_blend: playerPriorBlend,
      used_npxg90: reliableGoalRates.get(p.player_id),
      used_xa90: reliableAssistRates.get(p.player_id),
      rate_source: sourceParts.join("|"),
      attacking_rate_bound_applied: lowSampleRateBound,
...
export function selectLeagueRateMaps({
  currentSeasonRates = {}, priorSeasonRates = {}, configuredRates = {},
  currentSeasonFinished = 0, minimumFinishedMatches = 10,
} = {}) {
  const positions = ["GKP", "DEF", "MID", "FWD"];
  const usableRateMap = (map, key) => positions.every((pos) => {
    const value = Number(map?.[key]?.[pos]);
    if (!Number.isFinite(value) || value < 0) return false;
    if ((key === "npxg90" || key === "xa90") && pos !== "GKP") return value > 0;
    return value > 0 || pos === "GKP";
  });
  const pick = (key) => {
    if (currentSeasonFinished >= minimumFinishedMatches && usableRateMap(currentSeasonRates, key)) {
      return currentSeasonRates[key];
    }
    if (usableRateMap(priorSeasonRates, key)) return priorSeasonRates[key];
    if (usableRateMap(configuredRates, key)) return configuredRates[key];
    throw new Error(`No usable league ${key} priors; refusing to create zero-rate player templates`);
  };
  return {
    npxg90: pick("npxg90"),
    xa90: pick("xa90"),
    cbit90: pick("cbit90"),
    recoveries90: pick("recoveries90"),
  };
}
