...
function goalEnvironmentFromPowers(
  homePower,
  awayPower,
  leagueMeanGoals,
  homeAdvantage,
  ...
  const advantage = Math.min(1.3, Math.max(0.8, Number(homeAdvantage) || 1));
  const baseHome = leagueMeanGoals * advantage / (1 + advantage);
  const baseAway = leagueMeanGoals / (1 + advantage);
  const minimumLambda = 0.2;
  const maximumLambda = leagueMeanGoals * 1.10;

  let lambdaHome = Math.min(
    maximumLambda,
    Math.max(minimumLambda, baseHome * soften(homePower)),
  );
  let lambdaAway = Math.min(
    maximumLambda,
    Math.max(minimumLambda, baseAway * soften(awayPower)),
  );

  const minimumTotal = leagueMeanGoals * 0.70;
  const maximumTotal = leagueMeanGoals * 1.45;
  let total = lambdaHome + lambdaAway;

  if (total < minimumTotal && total > 0) {
    const scale = minimumTotal / total;
    lambdaHome = Math.min(maximumLambda, lambdaHome * scale);
    lambdaAway = Math.min(maximumLambda, lambdaAway * scale);
    total = lambdaHome + lambdaAway;
  }

  if (total > maximumTotal) {
    const homeReducible = Math.max(0, lambdaHome - minimumLambda);
    const awayReducible = Math.max(0, lambdaAway - minimumLambda);
    const reducible = homeReducible + awayReducible;
    const allowedReducible = maximumTotal - 2 * minimumLambda;
    const scale = reducible > 0
      ? Math.min(1, allowedReducible / reducible)
      : 1;

    lambdaHome = minimumLambda + homeReducible * scale;
    lambdaAway = minimumLambda + awayReducible * scale;
  }
...
export function fallbackGoalEnvironmentForTeams({
  ...
  if ([...Object.values(values), ...Object.values(means)].every((value) => value !== null)) {
    ...
    return goalEnvironmentFromPowers(
      homePower,
      awayPower,
      leagueMean,
      positiveFinite(homeAdvantage) || 1,
      "team-component-strength-fallback",
    );
  }
