...
import { applyResolvedTeams } from "./resolved_teams.mjs";
...
export async function loadModel(core) {
  ...
  /* Resolve transfers before any fixture, coverage or club-limit logic reads team_id. The lineup snapshot is
     allowed to override the stored player row only for the gameweek it describes; otherwise the team recorded
     by the selected projection generation is used. */
  ...
  const resolvedTeamChanges = applyResolvedTeams({
    players: core.players,
    teamById: core.teamById,
    projections,
    lineupOverrides: lineupApplies ? lineupResolution.teamOverrideByFplId : new Map(),
  });
  ...
  return {
    ...
    resolvedTeamChanges,
    projectedGws: [...new Set(projectionRuntime.selectedRows.map((row) => Number(row.gw)).filter(Number.isFinite))].sort((a, b) => a - b),
    gw,
  };
}
