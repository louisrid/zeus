...
import { collectAllPages } from "./paginated_read.mjs";
...
async function allProjectionRows(supabase, fromGw, toGw) {
  try {
    const result = await collectAllPages((from, pageSize) => supabase.from("projections")
      .select("*", from === 0 ? { count: "exact" } : undefined)
      .gte("gw", fromGw).lte("gw", toGw).range(from, from + pageSize - 1), {
      label: "projections",
      pageSize: 1000,
      maxRows: 100000,
    });
    ...
    throw projectionReadError(error, "projections");
  }
}
...
export async function loadModel(core) {
  ...
  // Prefer a complete coherent eight-gameweek generation. Until the first
  // production generation exists, retain the last valid per-gameweek output
  // instead of taking every product page offline.
  const expectedGameweeks = [...new Set((core.fixtures || [])
    .map((fixture) => Number(fixture.gw)).filter((fixtureGw) => fixtureGw >= 1 && fixtureGw <= 8))]
    .sort((a, b) => a - b);
  const expectedPlayerIdsByGameweek = new Map(expectedGameweeks.map((fixtureGw) => {
    const clubs = new Set((core.fixtures || []).filter((fixture) => Number(fixture.gw) === fixtureGw)
      .flatMap((fixture) => [Number(fixture.home_team), Number(fixture.away_team)]));
    return [fixtureGw, new Set(core.players.filter((player) => clubs.has(Number(player.team_id))).map((player) => Number(player.id)))];
  }));

  let projectionRuntime;
  try {
    projectionRuntime = buildProjectionRuntime(projRows, {
      currentGw: gw, idToFpl, expectedGameweeks, expectedPlayerIdsByGameweek,
    });
  } catch (error) {
    if (error?.code !== "NO_COMPLETE_PROJECTION_GENERATION") throw error;
    projectionRuntime = buildProjectionRuntime(projRows, { currentGw: gw, idToFpl });
  }
  ...
  const lineupResolution = resolveLineups(
    LINEUPS.clubs, core.players, Object.values(core.teamById),
  );
  ...
  const lineupRoles = lineupApplies
    ? lineupRolesOf(lineupResolution, core.players)
    : new Map();
