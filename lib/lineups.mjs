...
export function resolveLineups(clubs, players, teams) {
  ...
  return { byClub, startingIds, clubsCovered, unmatched };
}
...
export const LINEUP_MINUTES = {
  /* A PUBLISHED ELEVEN IS EVIDENCE, NOT A HINT.
   *
   * These were 0.94 and 88 minutes, which quietly shaved a tenth off every named starter and, worse, left room
   * for the reader to wonder whether the model believed the team news at all. If a player is named in a
   * published eleven he is starting: 1.0 and a full match. Louis has said this repeatedly about players like
   * Mosquera, who has no competition for his place, and he is right. Rotation risk belongs to players whose
   * line-up is unknown, not to players whose line-up has been published. */
  starter: { p_start: 1, exp_min_start: 90, p_cameo: 0, exp_min_cameo: 0 },
...
export function minutesWithLineups(clubs, existing, players, teams) {
  const { byClub, startingIds } = resolveLineups(clubs, players, teams);
