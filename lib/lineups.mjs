...
export function resolveLineups(clubs, players, teams) {
  ...
  for (const row of clubs || []) {
    ...
    /* An explicit id in `ids` is the strongest possible signal: it names one FPL element and cannot be
       confused by two players sharing a surname. It is a hint rather than a requirement, though. If the
       stored player list has not yet caught up and that id is absent, falling straight through to null
       would drop a named starter and silently invalidate the whole club. So an unknown id degrades to
       ordinary name resolution instead, which is exactly the behaviour of a file carrying no ids at all. */
    const lines = row.rows.map((line) => line.map((name) => {
      const forced = row.ids && row.ids[name];
      const player = (forced ? players.find((p) => p.fpl_id === forced) : null)
  ...
  /* A player named for a new club can still carry his old team_id before the upstream player list catches
     up. One predicted-XI occurrence therefore supplies a temporary engine team override.

     TRANSFERS NEED SOURCE RECENCY TO BEAT STALE STORED CLUB DATA. Lacroix appeared in both Palace's older
     predicted XI and Chelsea's newer one while the players table still said Palace. Preferring the stored club
     kept the obsolete occurrence and made the current club lose its player and projection. When duplicate
     occurrences have different parseable update dates, the unique newest occurrence wins. Only when recency
     cannot decide do we prefer the one matching the stored club; a remaining tie is rejected rather than
     guessed. */
  const updatedAt = (row) => {
    const t = Date.parse(String(row?.updated || row?.captured || ""));
    return Number.isFinite(t) ? t : null;
  };
  for (const [fplId, hits] of occurrences) {
    if (hits.length <= 1) continue;

    const dated = hits.map((hit) => ({ hit, at: updatedAt(hit.row) })).filter((x) => x.at !== null);
    let keep = null;
    if (dated.length) {
      const newest = Math.max(...dated.map((x) => x.at));
      const newestHits = dated.filter((x) => x.at === newest);
      if (newestHits.length === 1) keep = newestHits[0].hit;
    }
    if (!keep) {
      const currentClubHits = hits.filter((hit) => hit.club && hit.entry.player?.team_id === hit.club.id);
      if (currentClubHits.length === 1) keep = currentClubHits[0];
    }
...
/* A formation is also useful role evidence for a player with no previous PL
   sample. It is only a fallback: a role derived from real history still wins. */
export function lineupFootballRolesOf(resolution) {
  const roles = new Map();
  for (const { lines } of resolution?.byClub?.values?.() || []) {
    const lastLine = Math.max(0, lines.length - 1);
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      for (const entry of lines[lineIndex] || []) {
        ...
        if (position === "GKP") role = "goalkeeper";
          ...
          if (lineIndex === 2 && lines.length >= 4) role = "holding_midfielder";
          else if (lineIndex >= lastLine - 1) role = "attacking_midfielder";
          else role = "box_to_box_midfielder";
        } else if (position === "FWD") role = "focal_striker";
