...
import { nextFixturesForTeam } from "./fixture-outlook.mjs";
...
export function sb() {
  if (!client) {
    /* A missing key is a plain error the page's error card can show, not a crash inside the Supabase
       constructor that takes the whole page down before any component renders. */
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      throw new Error("The database is not configured for this site.");
    }
...
export async function loadCore() {
  ...
  const [teamsRes, playersRes, gwsRes] = await Promise.all([
    /* Current clubs only. The archived 2025/26 rows share this table and were coming through unfiltered,
       so relegated clubs appeared in every club dropdown as though they were still in the league. The
       players query already excludes archived rows; the teams query now does the same. */
    supabase.from("teams").select("*").not("archive", "is", true),
  ...
  const { data: fixtureRows, error: fe } = await supabase
    ...
    /* The window has to reach at least as far as points are served, or the gameweek pickers stop short
       of the projection and the last weeks of it can never be selected. Eight weeks is the minimum
       either way, so a mid-season refresh that extends the horizon widens this automatically. */
    .gte("gw", currentGw).lte("gw", Math.max(currentGw + 7, EXTERNAL_XPTS_GW_TO)).order("kickoff_utc");
...
export function nextFixtures(fixtures, teamById, teamId, n, fromGw = null) {
  return nextFixturesForTeam(fixtures, teamById, teamId, n, fromGw);
}
...
export function templateSquad(players, budget = SQUAD_BUDGET, scoreOf = null) {
  ...
  if (combined.curve[B] < 0) return finishTemplate(improve(greedyFifteen(players, budget), players, budget), budget, scoreOf);
  ...
  if (picked.length !== 15) return finishTemplate(improve(greedyFifteen(players, budget), players, budget), budget, scoreOf);
  ...
  return finishTemplate(own(a) >= own(b) ? a : b, budget, scoreOf);
}
...
function repairClubLimit(picked, players) {
  ...
  for (let guard = 0; guard < 30; guard++) {
    ...
    if (!over) break;
    const club = over[0];
    const offenders = out.filter((p) => String(p.team) === club).sort((a, b) => (a.own || 0) - (b.own || 0));
    /* A row with no club counts under the key "undefined", which matches nothing when the offenders are
       looked back up, leaving nothing to drop and throwing on the next line. That crashed the whole
       dashboard rather than degrading, so an unresolvable group is left alone instead. */
...
/* WHICH FIFTEEN VERSUS WHICH ELEVEN.
 *
 * These are two different questions and they were being answered with the same input. The fifteen is
 * chosen by ownership, which is the whole definition of a template: it is the squad most managers hold.
 * The eleven, though, is a decision, and seating it by ownership put Pedro Porro on 0.0 expected points
 * into the side while Ndiaye on 4.1 sat on the bench. Nobody would pick that.
 *
 * So the fifteen stays exactly as it was, and the eleven is now the best legal eleven inside it by
 * expected points, with the armband on the highest scorer. Pass scoreOf and it sorts by points; leave it
 * out and it falls back to ownership, so every existing caller behaves as before.
 */
function finishTemplate(picked, budget, scoreOf = null) {
  const rank = scoreOf
    ? (p) => { const v = Number(scoreOf(p)); return Number.isFinite(v) ? v : -1; }
    : (p) => Number(p.own) || 0;
  const gks = picked.filter((p) => p.position === "GKP").sort((a, b) => rank(b) - rank(a));
  const outfield = picked.filter((p) => p.position !== "GKP").sort((a, b) => rank(b) - rank(a));
  ...
  /* THE BENCH IS AN ORDER OF PREFERENCE, NOT A LEFTOVER PILE.
   *
   * This kept whatever order the players happened to be picked in, so the three outfield substitutes were
   * effectively arbitrary. They are a queue: if a starter does not play, the first of them comes on, so
   * the best should be first. The reserve keeper is not part of that queue at all, since he can only
   * replace the other keeper, and the game fixes him in the first slot. */
  const bench = picked.filter((p) => !xi.includes(p));
  bench.sort((a, b) => {
    const aKeeper = a.position === "GKP";
    const bKeeper = b.position === "GKP";
    if (aKeeper !== bKeeper) return aKeeper ? -1 : 1;
    if (aKeeper) return 0;
    return rank(b) - rank(a);
  });
  const order = { GKP: 0, DEF: 1, MID: 2, FWD: 3 };
  xi.sort((a, b) => order[a.position] - order[b.position]);
  /* The armband goes on the best of the eleven, and the vice on the next best, which is the only
     sensible reading of a squad shown with expected points on every shirt. Without a scorer there is no
     basis for either, so neither is set rather than guessed from ownership. */
  const byRank = [...xi].sort((a, b) => rank(b) - rank(a));
  const captainOf = scoreOf && byRank.length ? byRank[0] : null;
  const viceOf = scoreOf && byRank.length > 1 ? byRank[1] : null;
  const same = (a, b) => a && b && a.fpl_id === b.fpl_id && a.position === b.position;

  const fifteen = [...xi, ...bench].map((p) => ({
    ...p,
    flag: p.status !== "a",
    captain: same(p, captainOf),
    vice: same(p, viceOf),
  }));
