...
function unavailableQuery() {
  const result = {
    data: null,
    count: null,
    error: new Error("The database is not configured in this environment."),
  };
  let query;
  query = new Proxy({}, {
    get(_target, property) {
      ...
      if (property === "finally") return (handler) => Promise.resolve(result).finally(handler);
      return () => query;
    },
  });
  return query;
