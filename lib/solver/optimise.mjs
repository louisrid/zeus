...
export function optimiseSquad(squad, xpOf, opts = {}) {
  ...
  const requiredIds = new Set(opts.requiredStarterIds || []);
  ...
  for (const [key, st] of shapes) {
    ...
    const required = players.filter((p) => requiredIds.has(p.fpl_id));
    if (["GKP", "DEF", "MID", "FWD"].some((pos) => required.filter((p) => p.position === pos).length > st[pos])) continue;
    const xi = ["GKP", "DEF", "MID", "FWD"].flatMap((pos) => {
      const pinned = byPos[pos].filter((p) => requiredIds.has(p.fpl_id));
      const ranked = byPos[pos].filter((p) => !requiredIds.has(p.fpl_id));
      return [...pinned, ...ranked.slice(0, st[pos] - pinned.length)];
    });
  ...
  return {
    ...
    benchOrder: bench.map((p) => p.fpl_id),
