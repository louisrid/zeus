...
export function optimiseSquad(squad, xpOf, opts = {}) {
  ...
  for (const [key, st] of shapes) {
    // A shape is only available if the squad actually holds enough of each position and can seat every lock.
    if (["GKP", "DEF", "MID", "FWD"].some((pos) => byPos[pos].length < st[pos])) continue;
    let feasible = true;
    const xi = [];
    for (const pos of ["GKP", "DEF", "MID", "FWD"]) {
      const required = byPos[pos].filter((player) => requiredStarterIds.has(Number(player.fpl_id)));
      if (required.length > st[pos]) { feasible = false; break; }
      const requiredSet = new Set(required.map((player) => player.fpl_id));
      xi.push(...required, ...byPos[pos].filter((player) => !requiredSet.has(player.fpl_id)).slice(0, st[pos] - required.length));
    }
    if (!feasible || [...requiredStarterIds].some((id) => !xi.some((player) => Number(player.fpl_id) === id))) continue;
  ...
  return {
    ...
    benchOrder: bench.map((player) => player.fpl_id),
