...
export function buildSquadForRange({
  ...
  /* A ceiling on what the two goalkeepers may cost together. Null is no ceiling. */
  goalkeeperBudget = null,
  ...
  /* The goalkeeper cap, if one was asked for. Declared here because the evaluator, the swap passes and the
     pool filter all read it. */
  const goalkeeperCap = Number.isFinite(Number(goalkeeperBudget)) && Number(goalkeeperBudget) > 0
    ? Number(goalkeeperBudget) : null;
  ...
  if (hasBenchBoost && lockSet.size === 0 && keepSet.size === 0) {
    /* THE GOALKEEPER CAP, APPLIED BEFORE ANYTHING IS BUILT.
     *
     * A keeper who cannot be paired with even the cheapest other keeper under the cap can never be in a
     * legal answer, so he is out of the pool before the search starts. That is cheaper than building a
     * squad and repairing it afterwards, and it means the swap passes below never propose him either.
     * The cap itself is then enforced on every candidate squad, so the two together always fit. */
    const keeperPrices = (pool || []).filter((player) => player.position === "GKP" && !ignoreSet.has(idOf(player)))
      .map((player) => Number(player.price)).filter(Number.isFinite).sort((a, b) => a - b);
    const cheapestKeeper = keeperPrices[0] ?? 0;
    const allowedPool = (pool || []).filter((player) => !ignoreSet.has(idOf(player))
      && (goalkeeperCap === null || player.position !== "GKP" || lockSet.has(idOf(player))
        || Number(player.price) <= goalkeeperCap - cheapestKeeper + 1e-9));
  ...
  const evaluate = (candidatePlayers, structure) => {
    /* A squad whose keepers cost more than the cap is not a legal answer, so it scores as no answer. */
    if (goalkeeperCap !== null) {
      const keeperSpend = candidatePlayers.filter((player) => player.position === "GKP")
        .reduce((sum, player) => sum + Number(player.price), 0);
      if (keeperSpend > goalkeeperCap + 1e-9) return null;
    }
  ...
  /* BRING EACH SEED'S KEEPERS UNDER THE CAP BEFORE IT IS JUDGED.
   *
   * The seed builder knows nothing of the cap, so it can pair two keepers who each pass the pool filter
   * and together do not. Rejecting such a seed outright left nothing to build from and the whole run
   * failed with "cannot field a legal XI", which is not what happened. Instead the dearer keeper is
   * swapped for the best one that brings the pair under, and the seed goes on to be judged like any
   * other. The swap passes then keep the pair legal from there. */
  const withLegalKeepers = (players) => {
    if (goalkeeperCap === null) return players;
    const next = [...players];
    let guard = 4;
    while (guard-- > 0) {
      const keepers = next.filter((player) => player.position === "GKP");
      const spend = keepers.reduce((sum, player) => sum + Number(player.price), 0);
      if (keepers.length !== 2 || spend <= goalkeeperCap + 1e-9) return next;
      const owned = new Set(next.map(idOf));
      const clubs = clubCounts(next);
      let bestSwap = null;
      for (const outgoing of keepers) {
        if (lockSet.has(idOf(outgoing)) || keepSet.has(idOf(outgoing))) continue;
        const other = keepers.find((player) => idOf(player) !== idOf(outgoing));
        const room = goalkeeperCap - Number(other.price);
        for (const incoming of candidateByPosition.GKP || []) {
          if (owned.has(idOf(incoming)) || Number(incoming.price) > room + 1e-9) continue;
          const clubCount = (clubs.get(Number(incoming.team_id)) || 0)
            - (Number(incoming.team_id) === Number(outgoing.team_id) ? 1 : 0) + 1;
          if (clubCount > maxPerClub) continue;
          const loss = playerScore(outgoing) - playerScore(incoming);
          if (!bestSwap || loss < bestSwap.loss) bestSwap = { outgoing, incoming, loss };
        }
      }
      if (!bestSwap) return next;
      const index = next.findIndex((player) => idOf(player) === idOf(bestSwap.outgoing));
      next[index] = bestSwap.incoming;
    }
    return next;
  };

  const improve = (seed) => {
    let players = withLegalKeepers([...seed.players]);
    ...
    for (let pass = 0; pass < maxSwapPasses; pass += 1) {
      ...
      for (let index = 0; index < players.length; index += 1) {
        ...
        for (const incoming of candidateByPosition[outgoing.position] || []) {
          ...
          /* The pair of keepers must stay under the cap after this swap, whichever of them is moving. */
          if (goalkeeperCap !== null && outgoing.position === "GKP") {
            const otherKeeper = players.find((player) => player.position === "GKP" && idOf(player) !== outgoingId);
            if (otherKeeper && Number(incoming.price) + Number(otherKeeper.price) > goalkeeperCap + 1e-9) continue;
          }
  ...
  /* Keepers are brought under the cap before a seed is ranked, not only before it is improved. Ranking
     first and repairing second rejected every seed at the ranking step, so nothing reached the repair. */
  const rankedSeeds = seeds
    .map((seed) => ({ ...seed, players: withLegalKeepers([...seed.players]) }))
  ...
  /* Summed per player across every week of the range, counting only the weeks he actually starts, since a
     captain scores nothing from the bench. Highest total takes the armband, second highest the vice. */
  function rangeArmband(weekly, squadXi) {
    const totals = new Map();
    for (const week of weekly || []) {
      for (const starter of week.starters || []) {
        const id = Number(starter.fpl_id);
        totals.set(id, (totals.get(id) || 0) + (Number(starter.xpts) || 0));
      }
    }
    const eligible = (squadXi || [])
      .map((player) => ({ id: Number(player.fpl_id), total: totals.get(Number(player.fpl_id)) || 0 }))
      .sort((a, b) => b.total - a.total);
    if (!eligible.length) return { captain: null, vice: null };
    return {
      captain: eligible[0].id,
      vice: eligible[1] ? eligible[1].id : null,
    };
  }
  ...
  return {
    ...
    /* THE ARMBAND FOR THE RANGE, NOT FOR ITS FIRST WEEK.
     *
     * This returned the first gameweek's captain while the pitch beside it shows totals for the whole
     * range, so a squad built for GW6-12 wore the armband of whoever happened to be best in GW6. On a
     * seven week build that is close to arbitrary: it captained a midfielder on 38.8 over the range while
     * a forward on 43.6 sat next to him uncaptained.
     *
     * The weekly answer is still there in `weekly`, and it is the right one for stepping through
     * gameweeks. What the pitch needs, showing one squad and one set of totals, is the player who returns
     * most across the whole window. */
    ...rangeArmband(best.range.weekly, xi),
    weekly: best.range.weekly,
    total: best.range.total,
    /* Two decimals in the data. The screens round to one for display; the figure itself, which the API
       serves and the chats and Letta read, keeps its precision. Rounding here made every downstream
       comparison land on 0.1 steps and turned 1.57 into 1.6 before anyone could see it. */
    xp: Math.round(finite(best.range.total.net_xpts) * 100) / 100,
