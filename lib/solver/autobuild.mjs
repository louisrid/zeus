...
export function bestXI({
  ...
  /* A ceiling on what the two goalkeepers may cost between them. Null means no ceiling. */
...
export function improveSquad({
  ...
  /* A CAP ON THE TWO GOALKEEPERS COMBINED.
   *
   * The bench minimum says how little may be spent on reserves; nothing said how much may be spent on
   * the goalkeepers. A manager who wants two cheap keepers to fund the outfield had no way to ask for it,
   * and had to ban expensive keepers one name at a time. This is the same kind of repair pass the bench
   * minimum uses: while the two cost more than allowed, swap the dearer one for the best keeper that
   * brings the pair under, preferring the swap that loses the least. Locked keepers are never moved. */
  ...
  if (goalkeeperCap !== null) {
    let guard = 6;
    while (guard-- > 0) {
      const keepers = players.filter((player) => player.position === "GKP");
      if (keepers.length !== 2 || keepers.reduce((sum, player) => sum + price(player), 0) <= goalkeeperCap + EPS) break;
      const locked = new Set((locks || []).map(Number));
      const banned = new Set((ignores || []).map(Number));
      const movable = keepers.filter((player) => !locked.has(Number(player.fpl_id)));
      if (!movable.length) break;
      const inSquad = new Set(players.map((player) => Number(player.fpl_id)));
      const clubCounts = new Map();
      for (const player of players) clubCounts.set(player.team_id, (clubCounts.get(player.team_id) || 0) + 1);
      let best = null;
      for (const outgoing of movable) {
        const other = keepers.find((player) => player.fpl_id !== outgoing.fpl_id);
        const room = goalkeeperCap - price(other);
        const options = (pool || []).filter((player) => player.position === "GKP"
          && !inSquad.has(Number(player.fpl_id))
          && !banned.has(Number(player.fpl_id))
          && price(player) <= room + EPS
          && ((clubCounts.get(player.team_id) || 0) + (player.team_id === outgoing.team_id ? 0 : 1)) <= maxPerClub);
        for (const incoming of options) {
          const loss = xp(outgoing) - xp(incoming);
          if (!best || loss < best.loss) best = { outgoing, incoming, loss };
        }
      }
      if (!best) break;
      const index = players.findIndex((player) => player.fpl_id === best.outgoing.fpl_id);
      players[index] = { ...best.incoming, starting: Boolean(best.outgoing.starting) };
      changes.push({ out: best.outgoing.fpl_id, in: best.incoming.fpl_id, gain: -best.loss, reason: "goalkeeper budget" });
    }
  }
