...
export function bestXI({ pool, xpOf, locks = [], ignore = [], keep = [], budget = 100, maxPerClub = 3, startProbOf = null, minStart = 0.55 }) {
  ...
  const cand = (pool || []).filter((p) => Number.isFinite(price(p)) && price(p) > 0
    && (locks.includes(p.fpl_id) || !ignore.includes(p.fpl_id)));
  ...
  const startsEnough = (p) => {
    ...
    return s === null ? xp(p) > 0 : s >= minStart;
  };
