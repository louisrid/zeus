...
export function buildScorer({ projections, archivePer90, understat, envByTeam, envByTeamGw, perGw, leagueMeanGoals, goalPoints, assistPoints, appearancePoints, minutesForecasts, shrinkageNineties, positionMeans, promotionFactor, players, difficultyOf, hasFixture, engineShrinkNineties, penaltyTakers, teamQuality }) {
  ...
  const scoreOf = (p) => {
    ...
    if (a && a.nineties >= 2) {
      ...
      const shrunk = shrink(a.pointsPer90, a.nineties, p.position, Number(positionMean[p.position]) * qualityFor(p));
    ...
    if (nineties !== null && nineties > 0) {
      ...
      if (Number.isFinite(mean)) return round2(applyFixture(mean * qualityFor(p) + penaltyBonus(p), fx, appearancePoints) * nineties * avail * promoted(p));
    }
    return 0;
  };
  ...
  const scoreForGw = (p, gw) => {
    ...
    if (nineties !== null && nineties > 0) {
      const mean = Number(positionMean[p.position]);
      if (Number.isFinite(mean)) return round2(applyFixture(mean * qualityFor(p) + penaltyBonus(p), fx, appearancePoints) * nineties * avail * promo);
    }
    return null;
  };
