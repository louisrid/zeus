...
export function buildScorer({ projections, archivePer90, understat, envByTeam, envByTeamGw, perGw, leagueMeanGoals, goalPoints, assistPoints, appearancePoints, minutesForecasts, shrinkageNineties, positionMeans, promotionFactor, players, difficultyOf }) {
  ...
  const shrink = (rate, nineties, position, target = null) => {
  ...
  const scoreOf = (p) => {
    ...
    if (row && row.ep_mean !== null && row.ep_mean !== undefined) {
      ...
      return round2(shrink(engine, nineties, p.position, target) * promoted(p));
    }
    ...
    if (a && a.nineties >= 2) {
      const rate = shrink(a.pointsPer90, a.nineties, p.position) * fx * avail * promoted(p);
  ...
  const scoreForGw = (p, gw) => {
    ...
    if (series) {
      ...
      if (row && Number.isFinite(Number(row.ep_mean))) {
        ...
        if (played === null && nineties === 0 && mins.size > 0) return 0;
        const target = Number(positionMean[p.position]) * (played === null ? 1 : played);
        return round2(shrink(Number(row.ep_mean), nineties, p.position, target) * promoted(p));
      }
    }
    ...
    if (env) {
      ...
      // band fixtureMult uses, so the two routes are on one scale.
      fx = Math.max(0.55, Math.min(1.6, 1.6 - (Number(d) / 100) * 1.05));
    } else {
      return null;
    }
    const nineties = expectedNineties(p);
    ...
    const a = arch.get(p.fpl_id);
    if (a && a.nineties >= 2) {
      const rate = shrink(a.pointsPer90, a.nineties, p.position) * fx * avail * promo;
      return round2(nineties === null ? rate : rate * nineties);
    }
    const per90 = understatPer90(us.get(p.fpl_id));
    if (per90 !== null) {
