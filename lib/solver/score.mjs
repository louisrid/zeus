...
export function buildScorer({ projections, engineOnly = false, currentGw = null, archivePer90, understat, envByTeam, envByTeamGw, perGw, leagueMeanGoals, goalPoints, assistPoints, appearancePoints, minutesForecasts, shrinkageNineties, positionMeans, promotionFactor, players, difficultyOf, hasFixture, engineShrinkNineties, penaltyTakers, teamQuality, tuning, calibration, positionAttackMeans, minutesMeta, lineupVersion }) {
  ...
  const routeOf = (p, gw) => {
    ...
    if (engineOnly) {
      const current = proj.get(p.fpl_id);
      const future = gw !== undefined && currentGw !== null && Number(gw) > Number(currentGw);
      if (future && current && Number.isFinite(Number(current.ep_mean)) && (!hasFixture || hasFixture(p, gw))) {
        return "engine-anchored";
      }
  ...
  const currentGwFor = (p) => {
    ...
    return currentGw !== null && currentGw !== undefined ? Number(currentGw) : null;
  };

  const scoreForGwSmooth = (p, gw) => {
    const direct = scoreForGw(p, gw);
    if (engineOnly) {
      /* The live UI remains engine-only, but a missing stored FUTURE row must not turn every later gameweek
         blank. Anchor the same current engine projection to that fixture's difficulty. This is not a second
         player model or a historical fallback: the player's base number, minutes and role all come from the
         selected engine generation. Direct stored rows still win whenever the projection job has produced
         them. Current-gameweek coverage remains strict and cannot be anchored around a missing row. */
      if (direct !== null && direct !== undefined) return direct;
      if (currentGw !== null && Number(gw) > Number(currentGw)) return anchoredForGw(p, gw);
      return direct;
    }
...
export function provenanceLine(model) {
  if (model?.externalSource) {
    return `External xPTS import for GW1-GW${EXTERNAL_XPTS_GW_TO}. Imported minutes are metadata only and do not alter xPTS.`;
  }
