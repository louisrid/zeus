...
export function buildScorer({ projections, archivePer90, understat, envByTeam, envByTeamGw, perGw, leagueMeanGoals, goalPoints, assistPoints, appearancePoints, minutesForecasts, shrinkageNineties, positionMeans, promotionFactor, players, difficultyOf, hasFixture, engineShrinkNineties, penaltyTakers }) {
  ...
  const scoreOf = (p) => {
    ...
    if (a && a.nineties >= 2) {
      ...
      const shrunk = shrink(a.pointsPer90, a.nineties, p.position);
    ...
    if (nineties !== null && nineties > 0) {
      ...
      if (Number.isFinite(mean)) return round2(applyFixture(mean + penaltyBonus(p), fx, appearancePoints) * nineties * avail * promoted(p));
    }
    return 0;
  };
  ...
  const scoreForGw = (p, gw) => {
    ...
    if (nineties !== null && nineties > 0) {
      const mean = Number(positionMean[p.position]);
      if (Number.isFinite(mean)) return round2(applyFixture(mean + penaltyBonus(p), fx, appearancePoints) * nineties * avail * promo);
    }
    return null;
  };
