...
export function resolveMinutes({ base, lineup, status, earlySubShare = 0, confidence = 1, official = false, weeksAhead = 0 }) {
  ...
  /* Until a later lineup update replaces it, a validated predicted XI remains the model's selection
     assumption for future gameweeks. Carry only positive starter evidence forward at full strength,
     with no time decay, and never carry the negative not-named verdict into future weeks. */
  if (lineup === "carryStarter") {
    const carryWeeks = Math.max(1, num(weeksAhead, 1));
    const evidenceFloor = c;
    const pStart = Math.max(num(b.p_start), evidenceFloor);
    const pCameo = isGoalkeeper ? 0 : Math.min(num(b.p_cameo), Math.max(0, 1 - pStart));
    return {
      position,
      p_start: pStart, p_cameo: pCameo,
      p60: pStart * p60GivenStart, p60_given_start: p60GivenStart,
      exp_min_start: num(b.exp_min_start), exp_min_cameo: num(b.exp_min_cameo),
      wc_load_flag: Boolean(b.wc_load_flag),
      pre_lineup_p_start: num(b.p_start),
      minutes_source: "lineup-carryover",
      lineup_confidence: c, lineup_official: Boolean(official),
