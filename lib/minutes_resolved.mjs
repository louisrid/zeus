...
export const MINUTES_RESOLVER_VERSION = "resolved-6";
...
export function resolveMinutes({ base, lineup, status, earlySubShare = 0, confidence = 1, official = false, weeksAhead = 0 }) {
  ...
  /* The configured predicted XI remains the selection assumption for every future gameweek until a newer
     lineup file replaces it. Carry all named starters at 100% and retain only their player-specific
     conditional minutes. Team normalisation must lock this source exactly rather than dilute the eleven
     starters into one shared club probability. */
