...
export function resolveMinutes({ base, lineup, status, earlySubShare = 0 }) {
  const b = base || {};
  ...
  if (lineup === "starter" || lineup === "notNamed") {
    ...
    const pStart = num(L.p_start);
    const pCameo = num(L.p_cameo);
    return {
      p_start: pStart, p_cameo: pCameo,
      p60: pStart * p60GivenStart + pCameo * num(earlySubShare),
      p60_given_start: p60GivenStart,
      exp_min_start: num(L.exp_min_start), exp_min_cameo: num(L.exp_min_cameo),
      wc_load_flag: Boolean(b.wc_load_flag),
...
export function minutesInputVersion({ lineupVersion, status, chanceOfPlaying, minutesSource }) {
  const parts = [
...
export function lineupVersionOf(lineups) {
  ...
  return `${lineups.captured || "undated"}#${clubs}`;
}
