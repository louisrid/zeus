...
export const MINUTES_RESOLVER_VERSION = "resolved-5";
...
export function resolveMinutes({ base, lineup, status, earlySubShare = 0, confidence = 1, official = false, weeksAhead = 0 }) {
  const b = base || {};
  ...
  if (hardUnavailable(status)) {
    return {
      ...
      pre_lineup_p_start: num(b.p_start),
  ...
  /* Future gameweeks are forecasts, not locked lineups. The current XI remains useful provenance, but
     every named starter must retain his own forecast probability and cameo path. Setting all eleven to
     p_start=1 before team normalisation erased player-level information and collapsed them to one shared
     club scalar, which cut nailed players such as Haaland to roughly 50 expected minutes. */
  if (lineup === "carryStarter") {
    const carryWeeks = Math.max(1, num(weeksAhead, 1));
    const pStart = Math.max(0, Math.min(1, num(b.p_start)));
    const pCameo = isGoalkeeper
      ? 0
      : Math.max(0, Math.min(1 - pStart, num(b.p_cameo)));
    const expMinStart = Number.isFinite(Number(b.exp_min_start))
      ? Math.max(0, Math.min(90, Number(b.exp_min_start)))
      : 90;
    return {
      ...
      exp_min_cameo: num(b.exp_min_cameo),
      wc_load_flag: Boolean(b.wc_load_flag),
      pre_lineup_p_start: pStart,
      minutes_source: "lineup-carryover",
      lineup_confidence: c,
      lineup_official: Boolean(official),
      lineup_weeks_ahead: carryWeeks,
    };
  }
