...
export const MINUTES_RESOLVER_VERSION = "resolved-1";
...
export function resolveMinutes({ base, lineup, status, earlySubShare = 0, confidence = 1, official = false }) {
  ...
  /* A PREDICTED eleven is evidence, not a team sheet. One unofficial source was forcing p_start to 1 and
     90 minutes, so a tipster's guess outranked the minutes model outright. It is now blended with the
     forecast at a configurable confidence; only a confirmed official lineup uses certainty. */
  ...
  if (lineup === "starter" || lineup === "notNamed") {
    const L = LINEUP_MINUTES[lineup];
    const mix = (lineupVal, baseVal, fallback) => {
      const bv = Number.isFinite(Number(baseVal)) ? Number(baseVal) : num(fallback);
      return c * num(lineupVal) + (1 - c) * bv;
    };
    const pStart = Math.max(0, Math.min(1, mix(L.p_start, b.p_start, 0)));
    const pCameo = Math.max(0, Math.min(1 - pStart, mix(L.p_cameo, b.p_cameo, 0)));
    return {
      p_start: pStart, p_cameo: pCameo,
      p60: pStart * p60GivenStart + pCameo * num(earlySubShare),
      p60_given_start: p60GivenStart,
      exp_min_start: mix(L.exp_min_start, b.exp_min_start, L.exp_min_start),
      exp_min_cameo: mix(L.exp_min_cameo, b.exp_min_cameo, L.exp_min_cameo),
      wc_load_flag: Boolean(b.wc_load_flag),
      minutes_source: `lineup-${lineup}`,
      lineup_confidence: c,
  ...
  return {
    p_start: num(b.p_start), p_cameo: num(b.p_cameo),
    p60: num(b.p60),
    p60_given_start: p60GivenStart,
    exp_min_start: num(b.exp_min_start), exp_min_cameo: num(b.exp_min_cameo),
    wc_load_flag: Boolean(b.wc_load_flag),
...
export function lineupVersionOf(lineups) {
  ...
  return `${lineups.captured || "undated"}#${clubs}@${c}`;
}
...
export function lineupRolesOf({ byClub, startingIds }, players, confidentAt = 9) {
  const roles = new Map();
  for (const { club, lines } of byClub.values()) {
    if (!club) continue;
    const flat = lines.flat();
    for (const x of flat) if (x.player) roles.set(x.player.fpl_id, "starter");
    if (flat.filter((x) => x.player).length < confidentAt) continue;
