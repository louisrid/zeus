...
export function hydratePlanState(plan, gw, players = []) {
   ...
   * written still names him among the starters, so the man who replaced him reads as benched and the
   * transfer looks undone by OPTIMISE. The seat belongs to the slot, not the man: walk the transfers up
   * to this gameweek and translate each sold id to the id that replaced it.
   *
   * The translation is only used when it names a full XI this squad actually owns. Anything else keeps
   * the original reading exactly, because discarding a lineup that cannot be translated is what once
   * produced a ten-man XI with no captain. */
  const owned = new Set(raw.players.map((stored) => Number(stored.fpl_id)));
  const startingXI = PLAN_RULES.squadSize - 4;
  let configuredSet = Array.isArray(configured) ? new Set(configured.map(Number)) : null;
  if (Array.isArray(configured)) {
    ...
    if (replacedBy.size) {
      const carried = [...new Set(configured.map((id) => {
      ...
      if (carried.length === startingXI && carried.every((id) => owned.has(id))) {
        configuredSet = new Set(carried);
      }
    }
  }
