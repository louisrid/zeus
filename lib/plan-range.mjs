...
function exactRange(gwFrom, gwTo) {
  ...
  /* The upper bound is the served horizon, not a literal. A literal here rejected a range the data
     could answer, and the message named a window that was no longer true. */
  if (!Number.isInteger(from) || !Number.isInteger(to) || from < 1 || to > EXTERNAL_XPTS_GW_TO || to < from) {
    throw new Error(`The saved-squad range must be an exact inclusive range within GW1-GW${EXTERNAL_XPTS_GW_TO}.`);
  }
  return { from, to };
}
...
export function hydratePlanState(plan, gw, players = []) {
  ...
  /* A stored lineup outlives the squad it was written for. Transfer a player out and every week already
   * written still names him among the starters, so the man who replaced him is read as benched and the
   * transfer looks undone. The stored ids are only usable if they describe players this squad actually
   * owns in this gameweek AND still field a full XI; otherwise fall back to the seating held against
   * each player, which travels with the transfer. */
  /* A stored lineup outlives the squad it was written for. Transfer a player out and every week already
   * written still names him among the starters, so the man who replaced him is read as benched and the
   * transfer looks undone. The seat belongs to the slot, not the man: walk the transfers up to this
   * gameweek and translate each sold id to the id that replaced it. Only if that still fails to name a
   * full XI of owned players is the stored lineup discarded, and then the seating held against each
   * player is used, which travels with the transfer. */
  const owned = new Set(raw.players.map((stored) => Number(stored.fpl_id)));
  const startingXI = PLAN_RULES.squadSize - 4;
  const replacedBy = new Map();
  for (const [key, row] of Object.entries(plan?.weeks || {})) {
    if (Number(key) > Number(gw)) continue;
  ...
  const resolveId = (id) => {
    ...
    for (let hop = 0; hop < 15 && !owned.has(current) && replacedBy.has(current); hop += 1) {
      current = replacedBy.get(current);
    }
    return current;
  };
  const usableIds = Array.isArray(configured)
    ? [...new Set(configured.map(resolveId).filter((id) => owned.has(id)))]
    : null;
  const configuredSet = usableIds && usableIds.length === startingXI
    ? new Set(usableIds)
    : null;
...
export function applyOptimisedRangeToPlan(plan, result) {
  ...
  for (const week of result.weekly) {
    const canonicalGw = Number(week.gw);
    if (!Number.isInteger(canonicalGw) || canonicalGw < 1 || canonicalGw > 38) continue;
    next.weeks[String(canonicalGw)] = {
      ...(next.weeks[String(canonicalGw)] || next.weeks[canonicalGw] || {}),
