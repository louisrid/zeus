...
async function requestedGw(request) {
  ...
  const first = await rows("projections", "select=gw&order=gw.asc&limit=1");
  const fallback = Number(first[0]?.gw);
...
async function handle(request) {
  ...
  try {
    ...
    const [projectionRows, playerRows, teamRows] = await Promise.all([
      ...
      rows("players", "select=*&limit=2500"),
      rows("teams", "select=*&limit=100"),
    ]);
