...
import { fixtureEase, fixtureRatingBasis } from "../fixture-outlook.mjs";
import { buildFixtureDifficultyModel, buildTeamFixtureOutlooks, decorateFixturePerspective } from "../fixture-difficulty.mjs";
import {
...
function fixturePerspective({ team, opponent, home, scale, attackBasis, defenceBasis }) {
  const overall = team && opponent && scale?.ready ? scale.difficultyOf(opponent.id, home) : null;
  const attack = team && opponent
    ? fixtureEase({ mode: "ATTACK", fixture: { home }, opponent, basis: attackBasis, scale })
    : null;
  const defence = team && opponent
    ? fixtureEase({ mode: "DEFENCE", fixture: { home }, opponent, basis: defenceBasis, scale })
    : null;
  const attackEase = finite(attack?.ease);
  const defenceEase = finite(defence?.ease);
  return {
    team: pick(team, "short_name", "name") ?? null,
    team_id: finite(team?.id),
    opponent: pick(opponent, "short_name", "name") ?? null,
    opponent_id: finite(opponent?.id),
    venue: home ? "H" : "A",
    difficulty: finite(overall?.difficulty),
    difficulty_band: finite(overall?.band),
    difficulty_label: overall?.label ?? null,
    difficulty_basis: overall?.basis ?? scale?.basis ?? null,
    attack_ease: attackEase,
    attack_difficulty: attackEase === null ? null : 100 - attackEase,
    attack_basis: attack?.basis ?? null,
    defence_ease: defenceEase,
    defence_difficulty: defenceEase === null ? null : 100 - defenceEase,
    defence_basis: defence?.basis ?? null,
  };
}

export function buildFixturePayload({ fixtureRows = [], teamRows = [] } = {}) {
  const teamById = new Map(teamRows.map((team) => [Number(team.id), team]));
  const teamObject = Object.fromEntries(teamRows.map((team) => [Number(team.id), team]));
  const scale = buildOpponentScale(teamObject);
  const attackBasis = fixtureRatingBasis(teamObject, "ATTACK");
  const defenceBasis = fixtureRatingBasis(teamObject, "DEFENCE");
  const difficultyModel = buildFixtureDifficultyModel(teamRows);

  return fixtureRows.map((fixture) => {
    const home = teamById.get(Number(fixture.home_team)) || null;
    const away = teamById.get(Number(fixture.away_team)) || null;
    const homeView = decorateFixturePerspective({
      perspective: fixturePerspective({
        team: home, opponent: away, home: true, scale, attackBasis, defenceBasis,
      }),
      team: home,
      opponent: away,
      home: true,
    ...
    const awayView = decorateFixturePerspective({
      perspective: fixturePerspective({
        team: away, opponent: home, home: false, scale, attackBasis, defenceBasis,
      }),
      team: away,
      opponent: home,
      home: false,
    ...
    return {
      ...
      home_team_difficulty: homeView.difficulty,
      away_team_difficulty: awayView.difficulty,
      home_team_category: homeView.category,
      away_team_category: awayView.category,
      home_team_attack_difficulty: homeView.attack_difficulty,
      away_team_attack_difficulty: awayView.attack_difficulty,
      home_team_attack_category: homeView.attack_category,
      away_team_attack_category: awayView.attack_category,
      home_team_defence_difficulty: homeView.defence_difficulty,
      away_team_defence_difficulty: awayView.defence_difficulty,
      home_team_defence_category: homeView.defence_category,
      away_team_defence_category: awayView.defence_category,
      home_team_view: homeView,
      away_team_view: awayView,
    };
  });
}
...
export function buildSeasonProjectionRows({ playerRows = [], teamRows = [] } = {}) {
  ...
  const gate = serverLineupGate(players, teamRows);
  return buildExternalProjectionRows(players, {
    lineupStartingIds: gate.active ? gate.startingIds : null,
    lineupGateReport: gate.report,
  }).rows.sort((a, b) => a.gw - b.gw || b.xpts - a.xpts || a.name.localeCompare(b.name));
}

function briefPlayers({ playerRows = [], teamRows = [], gw }) {
  const players = livePlayers(playerRows, teamRows);
  const gate = serverLineupGate(players, teamRows);
  const built = buildExternalProjectionRows(players, {
    currentGw: gw,
    lineupStartingIds: gate.active ? gate.startingIds : null,
    lineupGateReport: gate.report,
  });
...
export function buildBrief({ playerRows = [], teamRows = [], gw }) {
  ...
  return {
    ...
    match_report: report,
    lineup_gate: report.lineup_gate || null,
    data: {
      gameweek: Number(gw),
      gw: Number(gw),
      generated_at: new Date().toISOString(),
...
async function handleSeasonView(request, view, params) {
  ...
  if (view === "fixtures") {
    ...
    const clubs = arrayParam(params.team ?? params.club ?? params.clubs)
      .map((value) => String(value).toLowerCase());
    if (clubs.length) {
      filteredFixtures = filteredFixtures.filter((fixture) => clubs.some((club) =>
        String(fixture.home_team).toLowerCase() === club
        || String(fixture.away_team).toLowerCase() === club
        || String(fixture.home_team_id) === club
        || String(fixture.away_team_id) === club));
    }
    const fixtureGwFrom = Math.max(1, Math.min(38, parseGw(params.gw_from ?? params.gw, 1)));
    const fixtureGwTo = Math.max(fixtureGwFrom, Math.min(38, parseGw(params.gw_to, 38)));
    filteredFixtures = filteredFixtures.filter((fixture) =>
      fixture.gw === null || (fixture.gw >= fixtureGwFrom && fixture.gw <= fixtureGwTo));

    return json({
      ok: true,
      view,
      fixture_count: filteredFixtures.length,
      total_fixture_count: fixtures.length,
      fixture_gameweeks: 38,
      gw_from: fixtureGwFrom,
      gw_to: fixtureGwTo,
      team_filter: clubs,
      xpts_gameweeks: [...EXTERNAL_XPTS_GAMEWEEKS],
      difficulty_scale: "0 = easiest, 100 = hardest",
      ease_scale: "0 = hardest, 100 = easiest",
      category_scale: { EASY: "0-39", MEDIUM: "40-59", HARD: "60-100" },
      difficulty_formula: "clamp(50 + 42 * (opponent normalised strength - assessed-team normalised strength) + venue adjustment, 0, 100)",
      difficulty_source: "shared ZEUS fixture schedule, team strengths and explicit home/away adjustments",
      team_outlooks: buildTeamFixtureOutlooks(filteredFixtures).filter((outlook) =>
        !clubs.length || clubs.some((club) => String(outlook.assessed_team).toLowerCase() === club
          || String(outlook.assessed_team_id) === club)),
      fixtures: filteredFixtures,
      usage: {
        all_fixtures: "/api/brief?view=fixtures",
        one_team: "/api/brief?view=fixtures&team=MCI",
        one_team_window: "/api/brief?view=fixtures&team=MCI&gw_from=1&gw_to=8",
      },
    });
  }
  ...
  return json({
    ...
    lineup_gate: built.report.lineup_gate || null,
    usage: {
      all_fixtures: "/api/brief?view=fixtures",
