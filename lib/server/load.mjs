...
async function all(client, table, select) {
  const result = await collectAllPages((from, pageSize) => client.from(table)
    .select(select, from === 0 ? { count: "exact" } : undefined)
    .range(from, from + pageSize - 1), { label: table, pageSize: 1000, maxRows: 100000 });
  return result.rows;
}
...
export async function loadForServer() {
  ...
  const expectedGameweeks = [...new Set(fixtures.map((fixture) => Number(fixture.gw))
    .filter((fixtureGw) => fixtureGw >= 1 && fixtureGw <= 8))].sort((a, b) => a - b);
  const expectedPlayerIdsByGameweek = new Map(expectedGameweeks.map((fixtureGw) => {
    const clubs = new Set(fixtures.filter((fixture) => Number(fixture.gw) === fixtureGw)
      .flatMap((fixture) => [Number(fixture.home_team), Number(fixture.away_team)]));
    return [fixtureGw, new Set(players.filter((player) => clubs.has(Number(player.team_id))).map((player) => Number(player.id)))];
  }));
