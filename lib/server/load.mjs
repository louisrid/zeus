...
import { resolveMinutes, lineupRolesOf, lineupVersionOf, minutesInputVersion } from "../minutes_resolved.mjs";
...
export async function loadForServer() {
  ...
  /* Engine rows, keyed the same way the browser keys them: this gameweek for the headline figure, and every
     gameweek for the per-week view. */
  const projections = new Map();
  const perGw = new Map();
  for (const r of projRows || []) {
    const pl = byInternalId.get(r.player_id);
    if (!pl) continue;
    if (Number(r.gw) === gw) projections.set(pl.fpl_id, r);
    if (!perGw.has(pl.fpl_id)) perGw.set(pl.fpl_id, []);
    const q = r.quantiles || {};
    perGw.get(pl.fpl_id).push({
      gw: Number(r.gw),
  ...
  const lineupRoles = lineupRolesOf(resolveLineups(LINEUPS.clubs, players, liveTeams), players);
  ...
  const scorer = buildScorer({
    projections, perGw, archivePer90, understat: new Map(),
