...
export async function loadForServer() {
  ...
  const idToFpl = new Map(players.map((p) => [p.fpl_id, p.fpl_id]));
  const archivePer90 = new Map();
  for (const r of priorRows) {
    const p = players.find((x) => x.fpl_id === r.player_id);
    if (!p) continue;
  ...
  for (const r of minRows) if (Number(r.gw) === gw) minutes.set(r.player_id, r);
  for (const [k, v] of minutesWithLineups(LINEUPS.clubs, minutes, players, teamRows)) minutes.set(k, v);

  const penaltyTakers = new Set();
  for (const r of dutyRows) {
    if (r.kind === "pen" && (r.rank === null || r.rank === undefined || Number(r.rank) <= 1)) {
      penaltyTakers.add(r.player_id);
    }
  }
  ...
  const teamQuality = new Map();
  {
    ...
    const mean = (fn) => { const v = teamRows.map(fn).filter((x) => x !== null); return v.length ? v.reduce((a, b) => a + b, 0) / v.length : null; };
  ...
  return { teamRows, teamById, players, fixtures, gw, scorer, scale, minutes, lineupsCaptured: LINEUPS.captured };
}
