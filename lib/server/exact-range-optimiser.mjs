...
/* xR: HOW MUCH xPTS TO GIVE UP, PER STARTER, PER GAMEWEEK, FOR DIFFERENTIATION.
 *
 * Rank is relative. Points most of the field also scores do not move you up, so a second objective
 * weights each player's projection by the share of the field that does not own him. The solver is not
 * allowed to chase that freely: it may give up at most XR_WINDOW points per starter per gameweek against
 * the best possible total, and must keep every other constraint. Tested against a strong simulated field,
 * 0.04 with b = 0.5 sits on the Pareto frontier at a quarter of the cost of the next setting. No tested
 * setting beat leaving xR off, so this is a bounded differentiation option, not a proven gain. Not
 * user-adjustable by design. */
export const XR_WINDOW = 0.04;
/* How hard ownership bites: xR = xPTS × (1 − XR_B × ownership/100). Half weight; b of 1 or more was
   dominated at every window on both test fields. Fixed, like the window. */
...
function exactRange(gwFrom, gwTo) {
  ...
  /* The upper bound follows the served horizon rather than a literal 8. With the literal in place,
     extending the projection was silently refused by the request validator while the data sat unused. */
...
function buildLp({
  ...
  /* THIRD PASS, WHEN ASKED FOR: MAXIMISE xR WITHIN A WINDOW OF THE BEST SCORE.
   *
   * Same rows, same binaries, same shape as the tie-break above. Two things differ. The weight on each
   * squad member is his projection scaled by (1 - ownership/100), summed over the range, and the floor
   * on the real objective is lowered by the window's budget rather than pinned at the maximum. xPTS
   * itself is never touched: it is the floor, not the thing being changed. */
  const xrLp = (objectiveFloorUnits) => {
    /* SAME SHAPE AS PASS ONE, REWEIGHTED.
     *
     * The spec summed xR over the fifteen. Tested, that spends the window on the bench even when every
     * player is owned identically and there is nothing to differentiate: with ownership flat, xR is a
     * constant multiple of xPTS, pass two should want exactly what pass one wanted, and instead it gave
     * up 2.36 of a 2.42 budget swapping reserves. Bench upgrades buy no rank.
     *
     * So pass two scores what pass one scores, starters and captain per week, each term reweighted by
     * (1 - ownership/100). With flat ownership the two objectives are proportional, the floor never
     * binds, and pass one's squad comes straight back: the degenerate case holds by construction. With
     * real ownership the window is spent only where differentiation actually exists. */
    const keepOf = (player) => {
      const share = ownershipOf ? ownershipOf(player) : 0;
      return 1 - XR_B * (Number.isFinite(share) ? Math.min(100, Math.max(0, share)) : 0) / 100;
    };
    const xrTerms = [];
    for (const player of eligible) {
      const id = idOf(player);
      const keep = keepOf(player);
      for (const gw of weeks) {
        const score = scoreUnits(scoreForGw(player, gw) * keep);
        const chip = chips.get(gw);
        if (chip === "benchboost") xrTerms.push(term(score, x(id)));
        else xrTerms.push(term(score, s(gw, id)));
        xrTerms.push(term(score * (chip === "triplecaptain" ? 2 : 1), c(gw, id)));
      }
    }
    return [
      "Maximize",
      ` xr: ${linearExpression(xrTerms)}`,
      "Subject To",
      ...rows,
      constraint("xpts_floor", objectiveTerms, ">=", objectiveFloorUnits),
      "Binary",
      ...binaries.map((name) => ` ${name}`),
      "End",
    ].join("\n");
  ...
  return { lp, tieBreakLp, xrLp, binaries, rowCount: rows.length, variableCount: binaries.length };
}

/* WHEN THE ANSWER IS "INFEASIBLE", SAY WHICH RULE MADE IT SO.
 *
 * A MILP that cannot be satisfied says only that it cannot; it does not say why. Every impossible
 * request looked identical on screen, "HiGHS did not prove a global optimum", whether the cause was two
 * locked keepers, a formation lock the locks could not fit, a goalkeeper cap the pinned keepers already
 * broke, or twenty-seven exclusions that left no legal fifteen. So on infeasibility the request is
 * re-run with one rule relaxed at a time, cheapest first, and the first relaxation that produces a
 * legal squad names the rule. It costs a few extra solves, only on failure, and turns a shrug into a
 * sentence someone can act on. */
export async function buildExactSquadForRange(options = {}) {
  const first = await solveExactSquadForRange(options);
  if (first.ok || !/Infeasible/i.test(String(first.error || ""))) return first;

  const relaxations = [
    ["the formation lock", { onlyFormation: null }],
    ["the goalkeeper spend cap", { maximumGoalkeeperSpend: null }],
    ["the minimum bench spend", { benchBudget: 0 }],
    ["the locked players", { locks: [], lockGameweeks: null, lockGameweeksByPlayer: null }],
    ["the players kept in the squad", { keep: [] }],
    ["the bench locks", { benchLocks: [], benchGameweeks: null, benchGameweeksByPlayer: null }],
    ["the change limit", { maximumChanges: null }],
    ["the exclusions", { ignores: [] }],
    ["the budget", { budget: 200 }],
  ];
  for (const [name, patch] of relaxations) {
    const changed = Object.keys(patch).some((key) => JSON.stringify(options[key] ?? null) !== JSON.stringify(patch[key] ?? null));
    if (!changed) continue;
    try {
      const relaxed = await solveExactSquadForRange({ ...options, ...patch });
      if (relaxed.ok) {
        return { ok: false, infeasible: true, cause: name,
          error: `No legal squad fits every rule together. Removing ${name} makes it possible, so that is the rule in conflict with the others.` };
      }
    } catch { /* keep looking */ }
  }
  return { ok: false, infeasible: true,
    error: "No legal squad fits every rule together, and no single rule is the cause on its own. Loosen two or more: fewer locks, fewer exclusions, or a wider budget." };
}

async function solveExactSquadForRange({
  ...
  onlyFormation = null,
  /* Optimise for rank movement rather than raw points, within the fixed window. See XR_WINDOW. */
  xr = false,
  ownershipOf = null,
} = {}) {
  const range = exactRange(gwFrom, gwTo);
  /* The rejected values are named. "must be within GW1-GW38" alone gave no way to tell an inverted
     range from an out-of-bounds one, and an inverted range arriving from a stale dropdown took a long
     time to find precisely because the message never said what it had been given. */
  if (!range) {
    return { ok: false,
  ...
  /* A cap the locked or kept keepers already break is a contradiction, and "Infeasible" is the wrong
     word for it. Say which players and what they cost, so the fix is obvious: raise the cap or unlock. */
  if (parsedMaximumGoalkeeperSpend !== null) {
    const pinned = new Set([...(locks || []), ...(keep || [])].map(Number));
    const pinnedKeepers = (pool || []).filter((player) => player.position === "GKP" && pinned.has(Number(player.fpl_id)));
    const pinnedSpend = pinnedKeepers.reduce((sum, player) => sum + (Number(player.price) || 0), 0);
    if (pinnedKeepers.length === 2 && pinnedSpend > parsedMaximumGoalkeeperSpend + 1e-9) {
      return { ok: false, error: `Your goalkeepers ${pinnedKeepers.map((player) => player.web_name).join(" and ")} cost £${pinnedSpend.toFixed(1)}m together, above the £${parsedMaximumGoalkeeperSpend.toFixed(1)}m cap. Raise the cap or unlock one of them.` };
    }
  }
  ...
  /* A PLAYER YOU ALREADY OWN IS NOT A CANDIDATE, HE IS A FACT.
   *
   * The pool below drops anyone flagged as injured or doubtful, which is right when choosing who to
   * buy and wrong when describing a squad that already exists. A doubtful player you own is still in
   * your team: FPL does not remove him, and neither can this.
   *
   * Dropping him made the transfer limit impossible to satisfy. That limit says "at least fifteen
   * minus N of your current squad must survive", counted against the squad you sent, but it can only
   * be met by players still in the pool. Lose one to a knock and the arithmetic can no longer be
   * satisfied by any squad at all, so every search came back Infeasible with nothing to indicate that
   * a fitness flag was the reason. It cost an evening once already.
   *
   * The current squad therefore joins the required set, exactly as keep already did. That is also why
   * passing the same fifteen through keep worked while current_squad did not: two routes to the same
   * constraint that disagreed about who exists. */
  const ownedSet = new Set(parsedCurrentSquad.map(Number).filter((id) => !ignoreSet.has(id)));
  /* TWO DIFFERENT IDEAS THAT MUST NOT BE THE SAME SET.
   *
   * requiredSet is a hard "this player is in the answer": it becomes an equality constraint further
   * down, so anything in it is pinned. lockSet, keepSet and benchSet all mean that.
   *
   * A player you own means something weaker: he is allowed to be considered, whatever his fitness
   * flag says, because he is already in your team and a flag cannot remove him. He must NOT be pinned,
   * or the search has nobody left to sell and every transfer comes back as no legal squad.
   *
   * Merging the two is exactly the mistake that broke this screen: fixing a squad that could not be
   * held silently pinned all fifteen and made every transfer impossible. They are separate now, and
   * selectableSet is only ever used to exempt a player from the pool filters below. */
  const selectableSet = new Set([...lockSet, ...keepSet, ...benchSet, ...ownedSet]);
  const requiredSet = new Set([...lockSet, ...keepSet, ...benchSet]);
  for (const id of selectableSet) {
  ...
  /* Only a genuinely REQUIRED player is worth failing over. A player Louis merely owns who has no
   * projection in this range cannot be selected, which simply means he is sold: refusing the whole
   * request over him made maximum_changes unusable whenever one owned player was unprojected, and
   * the caller got a confusing "Required player N is unavailable" instead of a transfer. This is the
   * same owned-versus-required distinction the comment above draws; the check had merged them again. */
  ...
  /* THE xR PASS.
   *
   * Runs only when asked. The floor is the best achievable score less the window's budget, so whatever
   * comes back is provably within that many points of the maximum; the downside is bounded by
   * construction, not by estimate. If the pass fails or is not proven optimal we keep the xPTS answer,
   * for the same reason the tie-break does: a refinement is never a reason to hand back something
   * unproven. Ownership missing on a player is filled with the pool median before this runs, because a
   * zero would make an unknown player look maximally differential. */
  const zMaxUnits = passOneObjectiveUnits;
  /* Pass one's fifteen, kept so the xR result can say who moved. */
  const maxSquadIds = new Set(eligible.filter((player) => selected(solution, `x_${idOf(player)}`)).map(idOf));
  let xrReport = null;
  if (xr) {
    const budget = XR_WINDOW * STARTERS * weeks.length;
    const floorUnits = Math.round(zMaxUnits - budget * SCORE_SCALE);
    try {
      const highs = await getHighs();
      const differentiated = highs.solve(model.xrLp(floorUnits), {
        output_flag: false,
        presolve: "on",
        mip_rel_gap: 0,
        mip_abs_gap: 0,
      });
      if (String(differentiated?.Status || "").toLowerCase() === "optimal") {
        assertIntegral(differentiated, model.binaries);
        /* The real score of the xR squad is the pass-one objective evaluated on it, which the floor row
           reports as its activity. Read it back rather than re-deriving it. */
        const xptsAchievedUnits = Number(differentiated?.Rows?.xpts_floor?.Primal
          ?? differentiated?.Rows?.find?.((row) => row.Name === "xpts_floor")?.Primal
          ?? zMaxUnits);
        ...
        xrReport = {
          ...
          xr_b: XR_B,
          xr_window: XR_WINDOW,
          xr_budget: Math.round(budget * 100) / 100,
          z_max: Math.round(zMaxUnits / SCORE_SCALE * 100) / 100,
          xpts_achieved: Math.round(xptsAchievedUnits / SCORE_SCALE * 100) / 100,
          xpts_given_up: Math.round((zMaxUnits - xptsAchievedUnits) / SCORE_SCALE * 100) / 100,
          xr_total: Math.round(Number(differentiated?.ObjectiveValue ?? 0) / SCORE_SCALE * 100) / 100,
        };
      }
    } catch {
      /* keep the xPTS answer */
    }
  }
  ...
  if (xrReport) {
    const chosenIds = new Set(selectedPlayers.map(idOf));
    const nameOf = (id) => eligible.find((player) => idOf(player) === id)?.web_name || String(id);
    xrReport.swaps_vs_max = {
      in: [...chosenIds].filter((id) => !maxSquadIds.has(id)).map(nameOf),
      out: [...maxSquadIds].filter((id) => !chosenIds.has(id)).map(nameOf),
    };
    const shares = selectedPlayers.map((player) => (ownershipOf ? ownershipOf(player) : null))
      .filter((value) => Number.isFinite(value));
    xrReport.avg_ownership = shares.length
  ...
  for (const gw of weeks) {
    ...
    /* Bench order is the one place bench players are ordered, and under xR it is ordered by xR: the
       reserve most likely to move rank comes on first. Keeper stays separate, as always. */
    const rankedBench = xrReport
      ? rankBenchForWeek(bench, (player, week) => {
        const share = ownershipOf ? ownershipOf(player) : null;
        const keep = 1 - XR_B * (Number.isFinite(share) ? Math.min(100, Math.max(0, share)) : 0) / 100;
        return finite(scoreForGw(player, week)) * keep;
      }, gw)
      : rankBenchForWeek(bench, scoreForGw, gw);
    ...
    weekly.push({
      ...
      bench_order_policy: xrReport ? "backup_gkp_first_then_outfield_descending_xr" : "backup_gkp_first_then_outfield_descending_xpts",
  ...
  /* With xR applied the squad is meant to score less than the maximum, so equality is the wrong proof.
     The proof becomes: the reconstruction sits inside the window. Below the floor means the pass returned
     something it was not allowed to, and the result is rejected the same way. The achieved figure is
     taken from the reconstruction, which is the number actually shown, rather than from a solver row. */
  /* The objective this squad is proved against: pass one's maximum normally, the reconstruction itself
     under xR, where the squad is meant to score less than the maximum. */
  let solverObjectiveUnits;
  if (xrReport) {
    solverObjectiveUnits = reconstructedObjectiveUnits;
    const floorUnits = Math.round(passOneObjectiveUnits - xrReport.xr_budget * SCORE_SCALE);
    if (reconstructedObjectiveUnits < floorUnits - 1) {
      return { ok: false, error: `xR pass returned a squad below the floor: ${reconstructedObjectiveUnits} < ${floorUnits}.` };
    }
    xrReport.xpts_achieved = Math.round(reconstructedObjectiveUnits / SCORE_SCALE * 100) / 100;
    xrReport.xpts_given_up = Math.round((passOneObjectiveUnits - reconstructedObjectiveUnits) / SCORE_SCALE * 100) / 100;
  } else {
    solverObjectiveUnits = tieBreakApplied
