...
function bestBenchBoostSplit({ squad, captain, scoreForGw, gw, xiMaximum, benchMinimum, onlyFormation }) {
...
export async function buildExactSquadForRange({
  ...
  locks = [], lockGameweeks = null, ignores = [], keep = [], budget = 100, benchBudget = 17,
  ...
  for (const gw of weeks) {
    ...
    if (chip === "benchboost") {
      ...
      if (!split) {
        return { ok: false, error: `GW${gw} Bench Boost XI tie-break found no legal split.` };
      }
