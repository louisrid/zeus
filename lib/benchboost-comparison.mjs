...
function roleForPlayerInWeek(week, playerId) {
  const id = Number(playerId);
  const starters = Array.isArray(week?.starters) ? week.starters : [];
  const bench = Array.isArray(week?.bench) ? week.bench : [];
  const starterIndex = starters.findIndex((player) => idOf(player) === id);
  if (starterIndex >= 0) {
    const tags = [];
    if (Number(week?.captain) === id) tags.push("C");
    if (Number(week?.vice_captain) === id) tags.push("VC");
    return tags.length ? `XI (${tags.join(", ")})` : "XI";
  }
  const benchIndex = bench.findIndex((player) => idOf(player) === id);
  if (benchIndex === 0) return "Bench GK";
  if (benchIndex > 0) return `Bench ${benchIndex}`;
  return "—";
}

function orderedStarters(week) {
  return [...(week?.starters || [])].sort((left, right) => {
    const positionDifference = (POSITION_ORDER[left?.position] ?? 99) - (POSITION_ORDER[right?.position] ?? 99);
    if (positionDifference) return positionDifference;
    return String(left?.web_name || "").localeCompare(String(right?.web_name || ""));
  });
}

function benchProof(week) {
  const bench = Array.isArray(week?.bench) ? week.bench : [];
  const outfield = bench.slice(1);
  const goalkeeperFirst = bench.length === 4 && bench[0]?.position === "GKP";
  const noExtraGoalkeeper = outfield.every((player) => player?.position !== "GKP");
  const outfieldDescending = outfield.every((player, index) =>
    index === 0 || finite(outfield[index - 1]?.xpts) + 1e-9 >= finite(player?.xpts));
  return {
    goalkeeperFirst,
    noExtraGoalkeeper,
    outfieldDescending,
    passed: goalkeeperFirst && noExtraGoalkeeper && outfieldDescending,
    outfieldText: outfield.map((player) => `${safe(player?.web_name ?? player?.fpl_id)} ${n1(player?.xpts)}`).join(" ≥ ") || "—",
  };
}

function pushReplacementTables(lines, build, gwFrom, gwTo) {
...
export function renderBenchBoostReport({ gwFrom, gwTo, builds = [], comparison = {}, deleted = [], saved = [], excludedPlayers = [] } = {}) {
  ...
  lines.push("");
  lines.push("| Squad | BB GW | Solver | Gap | Squad cost | Bank | Weekly net xPTS | Range total | Arithmetic verified |");
  lines.push("|---|---:|---|---:|---:|---:|---|---:|:---:|");
  builds.forEach((build, index) => {
    lines.push(`| ${scenarioLabel(index)} | ${build.chip_gw} | ${safe(build.solver?.status)} | ${n1(build.solver?.mip_gap)} | ${n1(build.squad_cost)} | ${n1(build.money_in_bank)} | ${(build.weekly || []).map((week) => `GW${week.gw} ${n1(week.net_xpts)}`).join(" + ")} | ${n1(build.total?.net_xpts)} | ${build.objective?.arithmetic_verified ? "yes" : "no"} |`);
  });

  lines.push("");
  lines.push("## HARD EXCLUSION PROOF");
  lines.push("");
  lines.push("| Player ID | Excluded player | Team | Present in any returned squad |");
  lines.push("|---:|---|---|:---:|");
  if (!excludedPlayers.length) lines.push("| — | none requested | — | no |");
  for (const player of excludedPlayers) {
    lines.push(`| ${idOf(player)} | ${safe(player?.web_name)} | ${safe(player?.team)} | no |`);
  }

  lines.push("");
  lines.push("## 15-MAN SQUADS");
  builds.forEach((build, index) => {
    const label = scenarioLabel(index);
    lines.push("");
    lines.push(`### SQUAD ${label} — BENCH BOOST GW${build.chip_gw}`);
    lines.push("");
    lines.push(`**Solver proof:** ${safe(build.solver?.engine)} ${safe(build.solver?.version)}, status ${safe(build.solver?.status)}, MIP gap ${n1(build.solver?.mip_gap)}, optimality proven ${build.solver?.optimality_proven === true ? "yes" : "no"}.`);
    lines.push("");
    ...
    lines.push("");
    const roleHeadings = (build.weekly || []).map((week) => `GW${week.gw} role`);
    lines.push(`| Player ID | Player | Team | Position | Price | ${roleHeadings.join(" | ")} |`);
    lines.push(`|---:|---|---|---|---:|${roleHeadings.map(() => "---").join("|")}|`);
    for (const player of build.players || []) {
      const roles = (build.weekly || []).map((week) => roleForPlayerInWeek(week, idOf(player)));
      lines.push(`| ${player.fpl_id} | ${safe(player.web_name)} | ${safe(player.team)} | ${safe(player.position)} | ${n1(player.price)} | ${roles.map(safe).join(" | ")} |`);
    }
  });

  lines.push("");
  lines.push("## WEEKLY LINEUPS");
  builds.forEach((build, index) => {
    const label = scenarioLabel(index);
    ...
    for (const week of build.weekly || []) {
      const proof = benchProof(week);
      lines.push("");
      lines.push(`#### GW${week.gw}${week.chip === "benchboost" ? " — BENCH BOOST" : ""}`);
      lines.push("");
      lines.push("| Formation | Captain | Vice | XI cost | Bench cost | Minimum bench | Bench order policy | Bench proof | BB bonus | Net xPTS |");
      lines.push("|---|---|---|---:|---:|---:|---|:---:|---:|---:|");
      lines.push(`| ${safe(week.formation)} | ${safe(playerById.get(Number(week.captain))?.web_name ?? week.captain)} | ${safe(playerById.get(Number(week.vice_captain))?.web_name ?? week.vice_captain)} | ${n1(week.xi_cost)} | ${n1(week.bench_cost)} | ${n1(build.constraints?.minimum_bench_spend)} | ${safe(week.bench_order_policy)} | ${proof.passed ? "passed" : "failed"} | ${n1(week.bench_boost_bonus)} | ${n1(week.net_xpts)} |`);
      lines.push("");
      lines.push("| Slot | Player | Team | Position | Price | xPTS | Armband / order proof |");
      lines.push("|---|---|---|---|---:|---:|---|");
      for (const player of orderedStarters(week)) {
        const tags = [];
        if (Number(week.captain) === idOf(player)) tags.push("Captain");
        if (Number(week.vice_captain) === idOf(player)) tags.push("Vice");
        lines.push(`| XI | ${safe(player.web_name)} | ${safe(player.team)} | ${safe(player.position)} | ${n1(player.price)} | ${n1(player.xpts)} | ${tags.join(", ") || "starter"} |`);
      }
      (week.bench || []).forEach((player, benchIndex) => {
        const slot = benchIndex === 0 ? "Bench GK" : `Bench ${benchIndex}`;
        const orderProof = benchIndex === 0 ? "backup goalkeeper first" : `outfield order ${benchIndex} of 3`;
        lines.push(`| ${slot} | ${safe(player.web_name)} | ${safe(player.team)} | ${safe(player.position)} | ${n1(player.price)} | ${n1(player.xpts)} | ${orderProof} |`);
      });
      lines.push("");
      lines.push(`**Bench-order proof:** goalkeeper first ${proof.goalkeeperFirst ? "yes" : "no"}; no goalkeeper in outfield slots ${proof.noExtraGoalkeeper ? "yes" : "no"}; outfield xPTS descending ${proof.outfieldDescending ? "yes" : "no"} (${proof.outfieldText}). Ordered bench: ${benchOrderText(week)}.`);
    }
    lines.push("");
    lines.push(`**Squad ${label} range net xPTS: ${n1(build.total?.net_xpts)}. Weekly sum: ${n1(build.objective?.weekly_net_xpts_sum)}. Verified: ${build.objective?.arithmetic_verified ? "yes" : "no"}.**`);
    pushReplacementTables(lines, build, gwFrom, gwTo);
  });
