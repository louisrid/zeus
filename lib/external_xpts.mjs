...
export const EXTERNAL_XPTS_GW_TO = Number(DATA.gw_to) || 8;
...
function rowForGw(record, gw) {
  ...
  return {
    gw: Number(gw),
    ep_mean: numberOrZero(record?.xpts?.[index]),
    r_exp_minutes: numberOrZero(record?.minutes?.[index]),
    r_p_start: Math.max(0, Math.min(1, numberOrZero(record?.minutes?.[index]) / 90)),
    r_p_cameo: null,
    minutes_source: "external-fplcopilot-import",
    rate_source: "external-fplcopilot-import",
    model_version: EXTERNAL_XPTS_MODEL_VERSION,
    computed_at: EXTERNAL_XPTS_IMPORTED_AT,
    source_name: record?.sourceName ?? null,
    match_status: record?.status ?? "zeroed-unmatched",
  };
}
...
export function buildExternalProjectionModel(players = [], options = {}) {
  ...
  for (const player of players) {
    ...
    const series = EXTERNAL_XPTS_GAMEWEEKS.map((gw) => rowForGw(record, gw));
    perGw.set(id, series);
    const current = rowForGw(record, currentGw);
  ...
  const recordOf = (player) => byFplId.get(Number(player?.fpl_id ?? player?.element ?? player?.id));
  const scoreForGw = (player, gw) => {
    const row = rowForGw(recordOf(player), Number(gw));
    return row ? row.ep_mean : null;
  };
  const minutesForGw = (player, gw) => {
    const row = rowForGw(recordOf(player), Number(gw));
    return row ? row.r_exp_minutes : null;
  };
  const startProbForGw = (player, gw) => {
    const value = minutesForGw(player, gw);
    return value === null ? null : Math.max(0, Math.min(1, value / 90));
  };
  const scoreOf = (player) => scoreForGw(player, currentGw);
  const sourceOf = (player) => recordOf(player)?.status ?? "zeroed-unmatched";
...
export function buildExternalProjectionRows(players = [], options = {}) {
  ...
  for (const player of players) {
    ...
    for (const gw of EXTERNAL_XPTS_GAMEWEEKS) {
      const index = gw - EXTERNAL_XPTS_GW_FROM;
      const xpts = numberOrZero(record?.xpts?.[index]);
      const expectedMinutes = numberOrZero(record?.minutes?.[index]);
      output.push({
        gw,
        ...
        xpts,
        xp: xpts,
        xpts_total_8gw: numberOrZero(record?.total),
        expected_minutes: expectedMinutes,
        start_probability: Math.max(0, Math.min(1, expectedMinutes / 90)),
