...
function rowForGw(record, gw, playerId, gate) {
  ...
  const predictedStart = gate.active ? gate.startingIds.has(Number(playerId)) : null;
  ...
  return {
    ...
    lineup_snapshot_gw: gate.active ? 1 : null,
    minutes_source: gate.active ? "external-import+predicted-lineup-gate" : "external-fplcopilot-import",
    rate_source: gate.active ? "external-import+predicted-lineup-gate" : "external-fplcopilot-import",
    model_version: EXTERNAL_XPTS_MODEL_VERSION,
    computed_at: EXTERNAL_XPTS_IMPORTED_AT,
    source_name: record?.sourceName ?? null,
    match_status: gate.active && !predictedStart
