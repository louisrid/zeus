...
/* METRICS YOU CAN FILTER ON BUT DO NOT WANT A COLUMN FOR.
 *
 * The table has a width budget and it is nearly spent: every column added is one the reader has to scan
 * past on every row, forever, to gain a number they may want once. Minutes played is exactly that. It is
 * the right thing to filter on, because a per-90 or a points total off two substitute appearances is
 * noise, and the wrong thing to give permanent space to.
 *
 * So the condition builder is fed a longer list than the table draws. Sorting and columns stay as they
 * were; the filters gain what they need. */
export const CONDITION_ONLY_KEYS = [
  { key: "MINUTES", label: "MINUTES PLAYED" },
  /* The club's average fixture difficulty across whatever gameweek range is selected, 1 easy to 5 hard.
     A filter rather than a column because it is a property of the club, so it would repeat identically
     down every row of a club's players and earn none of the width it took. */
  { key: "FDR", label: "FIXTURE DIFFICULTY" },
];

export const CONDITION_KEYS = [...SORT_KEYS, ...CONDITION_ONLY_KEYS];
...
export function formatMetric(key, value) {
  ...
  if (key === "MINUTES") return String(Math.round(number));
  if (key === "FDR") return number.toFixed(1);
  if (key === "XR") return number.toFixed(1);
