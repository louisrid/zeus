...
/* THIS SEASON'S DEFENSIVE NUMBERS, AND HOW HARD THE FIXTURES ARE.
 *
 * config/defcon-2026-27.mjs is LAST season: 2025/26 totals, captured once and never moved. It is the
 * right thing to judge a player's history by and the wrong thing to judge his current form by, and there
 * was no way to see the difference. The official bootstrap carries defensive_contribution_per_90 for the
 * season in progress, and the fixture list carries a difficulty rating for both sides of every match.
 * Both are free with the call this job already makes, and both go stale the same way projections do, so
 * they are written on the same six-hourly schedule rather than by hand. */
const DEFCON_LIVE_PATH = path.join(process.cwd(), "config", "defcon-live-2026-27.mjs");
const FDR_PATH = path.join(process.cwd(), "config", "fdr-2026-27.mjs");
const FIXTURES_URL = "https://fantasy.premierleague.com/api/fixtures/";
/* WHAT ACTUALLY HAPPENED, PER PLAYER, PER GAMEWEEK.
 *
 * Everything else this job writes is a forecast. The season so far was only reachable through
 * /api/actual-points, which asks the official API once per gameweek on every request: fine for a chat
 * fetching one range, far too slow for a table of six hundred players or a player page that wants the
 * whole season. It is the same data either way, so it is imported here and stored like the rest. */
const ACTUALS_PATH = path.join(process.cwd(), "config", "season-actuals-2026-27.mjs");
...
const DATA = `;
  ...
  /* THIS SEASON'S DEFCON PER 90.
   *
   * Straight from the bootstrap, which computes it the same way the game does, so there is no second
   * opinion to disagree with the official one. Minutes and starts travel with it because a per-90 built
   * on ninety minutes of football is noise and the reader has to be able to see that for himself: the
   * number is never hidden, but it is never presented as settled either. */
  const defconRows = [];
  for (const element of bootstrap?.elements || []) {
    const minutes = Number(element.minutes) || 0;
    const contribution = Number(element.defensive_contribution) || 0;
    const per90 = Number(element.defensive_contribution_per_90);
    defconRows.push({
      fpl_id: Number(element.id),
      ...
      nineties: Math.round((minutes / 90) * 100) / 100,
      per90: Number.isFinite(per90) ? per90 : (minutes > 0 ? Math.round((contribution / (minutes / 90)) * 100) / 100 : null),
    });
  }
  const defconPayload = {
    source: "Fantasy Premier League bootstrap-static",
    season: SEASON,
    captured: importedAt,
    note: "Defensive contribution per 90 for the season IN PROGRESS. config/defcon-2026-27.mjs holds last "
      + "season instead; the two are deliberately separate because early-season minutes make this one "
      + "volatile and last season's is settled.",
    minutes_caution: "A per-90 built on very few minutes says little. minutes and starts travel with every "
      + "row so a reader can judge that rather than being protected from it.",
    player_count: defconRows.length,
    rows: defconRows,
  };
  await writeFile(DEFCON_LIVE_PATH,
    `/* GENERATED by jobs/external_xpts_pull.mjs. Do not edit by hand. */\nconst DATA = ${JSON.stringify(defconPayload)};\nexport default DATA;\n`,
    "utf8");
  const withMinutes = defconRows.filter((row) => row.minutes > 0).length;
  console.log(`Wrote ${DEFCON_LIVE_PATH}`);
  console.log(`  ${defconRows.length} players, ${withMinutes} with minutes this season`);

  /* FIXTURE DIFFICULTY.
   *
   * The official list rates both sides of every match from 1 to 5, and the run of fixtures a club faces
   * matters more to a transfer than almost anything else on the site. Stored per club per gameweek so a
  ...
  await writeFile(FDR_PATH,
    `/* GENERATED by jobs/external_xpts_pull.mjs. Do not edit by hand. */\nconst DATA = ${JSON.stringify(fdrPayload)};\nexport default DATA;\n`,
    "utf8");
  const rated = Object.values(byClub).reduce((total, weeks) => total + Object.keys(weeks).length, 0);
  console.log(`Wrote ${FDR_PATH}`);
  console.log(`  ${Object.keys(byClub).length} clubs, ${rated} club-gameweeks rated`);

  /* Only gameweeks that have actually been played. Asking for a round that has not started returns a
     page of zeroes, and a zero that means "not played yet" stored next to a zero that means "played and
     scored nothing" is the kind of ambiguity that ends up on a screen as a fact. */
  const playedWeeks = (bootstrap?.events || [])
    .filter((event) => event.finished || event.is_current)
    .map((event) => Number(event.id))
    .filter((gw) => Number.isInteger(gw) && gw >= 1 && gw <= TOTAL_GWS)
    .sort((a, b) => a - b);

  const byPlayer = new Map();
  for (const gw of playedWeeks) {
    // eslint-disable-next-line no-await-in-loop
    const live = await getJson(LIVE_URL(gw), `Live scores for GW${gw}`);
  ...
  const actualRows = [];
  for (const [fplId, truth] of official) {
    const weeks = byPlayer.get(Number(fplId)) || {};
    const totals = Object.values(weeks).reduce((sum, week) => ({
      points: sum.points + week.points,
      minutes: sum.minutes + week.minutes,
      goals: sum.goals + week.goals,
      assists: sum.assists + week.assists,
      bonus: sum.bonus + week.bonus,
      starts: sum.starts + (week.minutes >= 60 ? 1 : 0),
      appearances: sum.appearances + (week.minutes > 0 ? 1 : 0),
    }), { points: 0, minutes: 0, goals: 0, assists: 0, bonus: 0, starts: 0, appearances: 0 });
    actualRows.push({
      fpl_id: Number(fplId),
      name: truth.web_name,
      club: truth.club,
      ...
      points_per_90: totals.minutes > 0
        ? Math.round((totals.points / (totals.minutes / 90)) * 100) / 100
        : null,
      weeks,
    });
  }
  actualRows.sort((a, b) => b.total_points - a.total_points);

  const actualsPayload = {
    source: "Fantasy Premier League live scores",
    season: SEASON,
    captured: importedAt,
    gameweeks_played: playedWeeks,
    note: "What each player actually scored, per gameweek. Only gameweeks that have started are here: a "
      + "week absent from a player means it has not been played, which is not the same as a week he "
      + "played and scored nothing.",
    player_count: actualRows.length,
    rows: actualRows,
  };
  await writeFile(ACTUALS_PATH,
    `/* GENERATED by jobs/external_xpts_pull.mjs. Do not edit by hand. */\nconst DATA = ${JSON.stringify(actualsPayload)};\nexport default DATA;\n`,
    "utf8");
  const scored = actualRows.filter((row) => row.total_points > 0).length;
  console.log(`Wrote ${ACTUALS_PATH}`);
  console.log(`  GW${playedWeeks.join(", GW")} · ${scored} players with points`);
