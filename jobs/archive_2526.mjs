...
async function main() {
  ...
  const { data: teamRows } = await supabase.from("teams").select("id, name, short_name");
  const teamByName = {};
  ...
  const { data: pRows } = await supabase.from("players").select("id, name");
  ...
  const { data: existingFx } = await supabase.from("fixtures").select("id, fpl_id").gte("fpl_id", OFFSET);
